...
static void add_filter_data_tail(struct kernel_grip_info* grip_info, uint8_t index,
                                 struct point_info point) {
    int i = 0, cnt = 0;
    struct coord_buffer* buf = NULL;
...
static void assign_filtered_data(struct kernel_grip_info* grip_info, uint8_t index,
                                 struct point_info* point) {
    int i = 0, cnt = 0;
...
static bool large_area_judged(struct kernel_grip_info* grip_info, uint16_t* grip_side,
    ...
    if (skip_handle_judge(grip_info, &cur_p)) return true;
...
static bool large_area_judged_curved(struct kernel_grip_info* grip_info, uint16_t* grip_side,
                                     struct point_info* points, int index) {
    ...
    if (is_exit && (TYPE_REJECT_DONE == grip_info->large_reject[index]) &&
        ...
        if (cur_p.tx_press == 0 && grip_info->first_point[index].tx_press != 0) {
            if (cur_p.rx_er <= long_side_para.normal_finger_thd_2 &&
                cur_p.rx_press <
                        long_side_para.large_palm_thd_1) {  // finger need to exit right now
                is_exit = true;
            }
        }
        if (cur_p.rx_press == 0 && grip_info->first_point[index].rx_press != 0) {
            if (cur_p.tx_er <= short_side_para.normal_finger_thd_2 &&
                cur_p.tx_press <
                        short_side_para.large_palm_thd_1) {  // finger need to exit right now
                is_exit = true;
            }
        }
    }
...
static enum large_judge_status large_shape_judged(struct kernel_grip_info* grip_info, uint16_t side,
                                                  struct point_info* points, int index) {
    uint16_t thd = 0;
        ...
        if (frame <= grip_info->large_corner_frame_limit) {
            ...
            if ((side >> TYPE_SHORT_CORNER_SIDE) & 0x01) {
                ...
                if (cur_p.y < grip_info->max_y / 2) {
                    ...
                    area_flag = grip_info->first_point[index].y >
                                grip_info->max_y - grip_info->large_hor_corner_width;
                }
            ...
            if ((side >> TYPE_LONG_CORNER_SIDE) & 0x01) {
                ...
                if (cur_p.x < grip_info->max_x / 2) {
                    ...
                    area_flag = grip_info->first_point[index].x >
                                grip_info->max_x - grip_info->large_ver_corner_width;
                }
...
static enum large_judge_status large_shape_judged_curved(struct kernel_grip_info* grip_info,
                                                         uint16_t side, struct point_info* points,
                                                         int index) {
    struct point_info cur_p = points[index];
    struct curved_judge_para long_side_para = grip_info->curved_long_side_para;
    struct curved_judge_para short_side_para = grip_info->curved_short_side_para;
    ...
    if (((side >> TYPE_LONG_SIDE) & 0x01) || ((side >> TYPE_SHORT_SIDE) & 0x01)) {
        if (delta_time_ms <= grip_info->large_detect_time_ms) {
            TPD_DETAIL("id:%d, rx:%d, tx:%d, rx_er:%d, tx_er:%d. (%d %d)\n", index, cur_p.rx_press,
                       cur_p.tx_press, cur_p.rx_er, cur_p.tx_er, cur_p.x, cur_p.y);
            if ((side >> TYPE_LONG_SIDE) & 0x01) {
                if (cur_p.tx_press == 0) {  // long side
                    if (cur_p.rx_press >= long_side_para.large_palm_thd_1 ||
                        (cur_p.rx_er >= long_side_para.large_palm_thd_2 &&
                         delta_time_ms <= grip_info->large_detect_time_ms / 5)) {
                        TPD_INFO(
                                "palm long size reject id:%d for rx:%d, tx:%d, rx_er:%d, "
                                "tx_er:%d.\n",
                                index, cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                        grip_info->large_finger_status[index] = TYPE_PALM_LONG_SIZE;
                        judge_status = JUDGE_LARGE_OK;
                    } else if (cur_p.rx_er >= long_side_para.edge_finger_thd &&
                               delta_time_ms <= grip_info->large_detect_time_ms / 5) {
                        grip_info->large_finger_status[index] = TYPE_EDGE_FINGER;
                    } else if (grip_info->first_point[index].rx_er - cur_p.rx_er >=
                               long_side_para.hold_finger_thd) {
                        grip_info->large_finger_status[index] = TYPE_HOLD_FINGER;
                    } else if (grip_info->first_point[index].rx_er <=
                                       long_side_para.normal_finger_thd_1 &&
                               cur_p.rx_er <= long_side_para.normal_finger_thd_2 &&
                               cur_p.rx_press <=
                                       long_side_para.normal_finger_thd_3) {  // finger need to exit
                                                                              // right now
                        grip_info->large_finger_status[index] = TYPE_NORMAL_FINGER;
                        judge_status = JUDGE_LARGE_TIMEOUT;
                    }
                } else if (cur_p.rx_press != 0 &&
                           !(grip_info->touch_dir == VERTICAL_SCREEN &&
                             grip_info->first_point[index].y <=
                                     grip_info->max_y / 2)) {  // long corner side
                    if (cur_p.rx_press >= long_side_para.large_palm_thd_1 ||
                        (cur_p.rx_er >= long_side_para.large_palm_thd_2 &&
                         delta_time_ms <= grip_info->large_detect_time_ms / 5)) {
                        TPD_INFO(
                                "large palm corner reject id:%d for rx:%d, tx:%d, rx_er:%d, "
                                "tx_er:%d.\n",
                                index, cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                        ...
                        judge_status = JUDGE_LARGE_OK;
                               ...
                               (cur_p.tx_er >= long_side_para.palm_thd_2 ||
                                cur_p.rx_er >= long_side_para.palm_thd_2)) {
                        TPD_INFO("palm corner reject id:%d for rx:%d, tx:%d, rx_er:%d, tx_er:%d.\n",
                                 index, cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                        grip_info->large_finger_status[index] = TYPE_PALM_CORNER;
                        judge_status = JUDGE_LARGE_OK;
                    } else if ((cur_p.rx_er / cur_p.rx_press >= long_side_para.small_palm_thd_1) &&
                               cur_p.rx_press <= long_side_para.small_palm_thd_2) {
            ...
            if ((side >> TYPE_SHORT_SIDE) & 0x01) {
                if (cur_p.rx_press == 0) {  // short side
                    if (cur_p.tx_press >= short_side_para.large_palm_thd_1 ||
                        (cur_p.tx_er >= short_side_para.large_palm_thd_2 &&
                         delta_time_ms <= grip_info->large_detect_time_ms / 5)) {
                        TPD_INFO(
                                "palm short size reject id:%d for rx:%d, tx:%d, rx_er:%d, "
                                "tx_er:%d.\n",
                                index, cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                        grip_info->large_finger_status[index] = TYPE_PALM_SHORT_SIZE;
                        judge_status = JUDGE_LARGE_OK;
                    } else if (cur_p.tx_er >= short_side_para.edge_finger_thd &&
                               delta_time_ms <= grip_info->large_detect_time_ms / 5) {
                        grip_info->large_finger_status[index] = TYPE_EDGE_FINGER;
                    } else if (grip_info->first_point[index].tx_er <=
                                       short_side_para.normal_finger_thd_1 &&
                               cur_p.tx_er <= short_side_para.normal_finger_thd_2 &&
                               cur_p.tx_press <=
                                       short_side_para.normal_finger_thd_3) {  // finger need to
                                                                               // exit right now
                        grip_info->large_finger_status[index] = TYPE_NORMAL_FINGER;
                        judge_status = JUDGE_LARGE_TIMEOUT;
                    }
                    ...
                    if (cur_p.tx_press >= short_side_para.large_palm_thd_1 ||
                        ...
                        judge_status = JUDGE_LARGE_OK;
                               ...
                               (cur_p.tx_er >= short_side_para.palm_thd_2 ||
                                cur_p.rx_er >= short_side_para.palm_thd_2)) {
                        TPD_INFO("palm corner reject id:%d for rx:%d, tx:%d, rx_er:%d, tx_er:%d.\n",
                                 index, cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                        grip_info->large_finger_status[index] = TYPE_PALM_CORNER;
                        judge_status = JUDGE_LARGE_OK;
                    } else if ((cur_p.tx_er / cur_p.tx_press >= short_side_para.small_palm_thd_1) &&
                               cur_p.tx_press <= short_side_para.small_palm_thd_2) {
                        grip_info->large_finger_status[index] = TYPE_SMALL_PALM_CORNER;
                    }
                }
            }
            ...
            if (grip_info->large_finger_status[index] == TYPE_EDGE_FINGER) {
                TPD_INFO("edge finger reject id:%d for rx:%d, tx:%d, rx_er:%d, tx_er:%d.\n", index,
                         cur_p.rx_press, cur_p.tx_press, cur_p.rx_er, cur_p.tx_er);
                judge_status = JUDGE_LARGE_OK;
...
static bool condition_area_judged(struct kernel_grip_info* grip_info, struct point_info* points,
                                  int index) {
    int thd = 0;
    struct list_head* pos = NULL;
    ...
    list_for_each(pos, &grip_info->condition_zone_list) {
        ...
        if ((condition_area->support_dir >> grip_info->touch_dir) & 0x01) {
            ...
            if ((grip_info->first_point[index].x <=
                 condition_area->start_x + condition_area->x_width) &&
                (grip_info->first_point[index].x >= condition_area->start_x) &&
                (grip_info->first_point[index].y <=
                 condition_area->start_y + condition_area->y_width) &&
...
static enum hrtimer_restart touch_up_timer_func(struct hrtimer* hrtimer) {
    ...
    work_id++;
    if (work_id >= TOUCH_MAX_NUM) work_id = 0;
    queue_work(g_tp->grip_info->grip_up_handle_wq, &g_tp->grip_info->grip_up_work[work_id]);
    return HRTIMER_NORESTART;
}

static int large_condition_handle(struct kernel_grip_info* grip_info, int obj_attention,
                                  struct point_info* points) {
        ...
        if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {  // finger down
            ...
            if (grip_info->large_out_status[i]) {  // if already exit large area judge
                if (grip_info->condition_out_status[i]) {
                    ...
                    grip_info->condition_out_status[i] =
                            condition_area_judged(grip_info, points, i);
                    if (grip_info->condition_out_status[i]) {
                        tmp_point = points[i];
                        assign_filtered_data(grip_info, i, &points[i]);
                        add_filter_data_tail(grip_info, i, tmp_point);
            ...
            if (!grip_info->large_out_status[i] &&
                ...
                if (dead_out) {
                    ...
                    hrtimer_start(&grip_info->grip_up_timer[i],
                                  ktime_set(0, grip_info->condition_updelay_ms * 1000000),
                                  HRTIMER_MODE_REL);
                } else {
...
static int curved_large_handle(struct kernel_grip_info* grip_info, int obj_attention,
                               struct point_info* points) {
    ...
    for (i = 0; i < TOUCH_MAX_NUM; i++) {
        if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {  // finger down
            ...
            if (!(((grip_info->obj_prev_bit & TOUCH_BIT_CHECK) >> i) &
                ...
                if (grip_info->large_point_status[i] ==
                    UP_POINT) {  // if there is some point touch down, mark previous down points no
                                 // need to make up point
            ...
            if (is_exit) {
                ...
                if (!grip_info->point_unmoved[i]) {  // means once in large judge area
                    ...
                    TPD_INFO("id:%d make up b:%d times.(%d %d)(%d %d %d %d)\n", i,
                             grip_info->makeup_cnt[i], points[i].x, points[i].y, points[i].tx_press,
                             points[i].rx_press, points[i].tx_er, points[i].rx_er);
                }
            } else if (TYPE_REJECT_DONE == grip_info->large_reject[i]) {
                obj_final = obj_final & (~(1 << i));  // if already reject, just mask it
            } else {
                ...
                if (JUDGE_LARGE_OK == judge_state) {
                    ...
                    if (!grip_info->point_unmoved[i]) {
                        ...
                        TPD_INFO("id:%d make up c:%d times.(%d %d)(%d %d %d %d)\n", i,
                                 grip_info->makeup_cnt[i], points[i].x, points[i].y,
                                 points[i].tx_press, points[i].rx_press, points[i].tx_er,
                                 points[i].rx_er);
                    }
                } else {
                    obj_final = obj_final & (~(1 << i));  // reject for continue detect
            ...
            if (!grip_info->large_out_status[i] &&
                (TYPE_REJECT_DONE == grip_info->large_reject[i])) {
                TPD_INFO("reject id:%d for large area.\n", i);
            } else if (grip_info->large_point_status[i] != DOWN_POINT_NEED_MAKEUP &&
                       !grip_info->large_out_status[i] &&
                       (TYPE_REJECT_HOLD == grip_info->large_reject[i])) {
                TPD_INFO("reject no need make up click point id:%d for large area.\n", i);
            } else if ((grip_info->first_point[i].time_ms - grip_info->lastest_down_time_ms <=
...
static int touch_elimination_handle(struct kernel_grip_info* grip_info, int obj_attention,
                                    struct point_info* points) {
...
int notify_prevention_handle(struct kernel_grip_info* grip_info, int obj_attention,
                             struct point_info* points) {
    int i = 0;
    ...
    for (i = 0; i < TOUCH_MAX_NUM; i++) {
        if (grip_info->grip_hold_status[i]) {  // handle hold id
            ...
            } else if (!grip_info->eli_reject_status[i]) {  // aviod hold points be cleared by next
                                                            // down frame
                obj_attention = obj_attention |
                                (1 << i);  // point[i].status is 0, means no down and no up report
            } else {
...
int kernel_grip_print_func(struct seq_file* s, struct kernel_grip_info* grip_info) {
    ...
    list_for_each(pos, &grip_info->dead_zone_list) {
        ...
        seq_printf(s,
                   "name:%24s, start_point:(%4d, %4d), width:(%4d, %4d), exit_thd:%4d, "
                   "support_dir:0x%02x, side:0x%02x.\n",
                   grip_area->name, grip_area->start_x, grip_area->start_y, grip_area->x_width,
                   grip_area->y_width, grip_area->exit_thd, grip_area->support_dir,
                   grip_area->grip_side);
    }
    seq_printf(s, "\n");
    ...
    list_for_each(pos, &grip_info->large_zone_list) {
        ...
        if (grip_info->is_curved_screen) {
            if (strstr(grip_area->name, "curved")) {
                seq_printf(s,
                           "name:%24s, start_point:(%4d, %4d), width:(%4d, %4d), exit_thd:%4d, "
                           "exit_tx_er:%4d, exit_rx_er:%4d, support_dir:0x%02x, side:0x%02x.\n",
                           grip_area->name, grip_area->start_x, grip_area->start_y,
                           grip_area->x_width, grip_area->y_width, grip_area->exit_thd,
                           grip_area->exit_tx_er, grip_area->exit_rx_er, grip_area->support_dir,
                           grip_area->grip_side);
            }
        } else {
            seq_printf(s,
...
large_ver_corner_width:%4d, large_hor_corner_width:%4d, large_corner_distance:%4d\n",
               ...
               grip_info->large_hor_corner_thd, grip_info->large_ver_corner_width,
               grip_info->large_hor_corner_width, grip_info->large_corner_distance);
    seq_printf(s, "\n");
    ...
    if (grip_info->is_curved_screen) {
        ...
        seq_printf(
                s,
                "edge_finger_thd:%4d, hold_finger_thd:%4d, normal_finger_thd_1:%4d, normal_finger_thd_2:%4d, normal_finger_thd_3:%4d\n\
    large_palm_thd_1:%4d, large_palm_thd_2:%4d, palm_thd_1:%4d, palm_thd_2:%4d, small_palm_thd_1:%4d, small_palm_thd_2:%4d\n",
                long_side_para->edge_finger_thd, long_side_para->hold_finger_thd,
                long_side_para->normal_finger_thd_1, long_side_para->normal_finger_thd_2,
                long_side_para->normal_finger_thd_3, long_side_para->large_palm_thd_1,
                long_side_para->large_palm_thd_2, long_side_para->palm_thd_1,
                long_side_para->palm_thd_2, long_side_para->small_palm_thd_1,
                long_side_para->small_palm_thd_2);
        seq_printf(s, "curved_large_judge_para_for_short_side:\n");
        seq_printf(
                s,
                "edge_finger_thd:%4d, hold_finger_thd:%4d, normal_finger_thd_1:%4d, normal_finger_thd_2:%4d, normal_finger_thd_3:%4d\n\
    large_palm_thd_1:%4d, large_palm_thd_2:%4d, palm_thd_1:%4d, palm_thd_2:%4d, small_palm_thd_1:%4d, small_palm_thd_2:%4d\n",
                short_side_para->edge_finger_thd, short_side_para->hold_finger_thd,
                short_side_para->normal_finger_thd_1, short_side_para->normal_finger_thd_2,
                short_side_para->normal_finger_thd_3, short_side_para->large_palm_thd_1,
                short_side_para->large_palm_thd_2, short_side_para->palm_thd_1,
                short_side_para->palm_thd_2, short_side_para->small_palm_thd_1,
                short_side_para->small_palm_thd_2);
        seq_printf(s, "\n");
    }
...
static int grip_area_add_modify(struct list_head* handle_list, char* in, bool is_add,
    ...
    if (is_add) {
        ...
        TPD_INFO("%s add: [%d, %d] [%d %d] %d %d %d.\n", grip_zone->name, grip_zone->start_x,
                 grip_zone->start_y, grip_zone->x_width, grip_zone->y_width, grip_zone->exit_thd,
                 grip_zone->support_dir, grip_zone->grip_side);

        if (grip_info->grip_handle_in_fw && op && op->set_fw_grip_area && g_tp &&
            !g_tp->loading_fw) {
            ...
            if (strstr(in, grip_zone->name)) {
                ...
                if (grip_info->is_curved_screen) {
                    ...
                    TPD_INFO("%s modify: [%d, %d] [%d %d] %d %d %d %d %d.\n", grip_zone->name,
                             grip_zone->start_x, grip_zone->start_y, grip_zone->x_width,
                             grip_zone->y_width, grip_zone->exit_thd, grip_zone->support_dir,
                             grip_zone->grip_side, grip_zone->exit_tx_er, grip_zone->exit_rx_er);
                } else {
...
static void skip_area_modify(struct kernel_grip_info* grip_info, char* in) {
    ...
    if (grip_info->grip_handle_in_fw && op && op->set_no_handle_area && g_tp && !g_tp->loading_fw) {
    ...
    while (i < str_cnt) {
        ...
        if (OBJECT_PARAMETER == object) {
            if (OPERATE_MODIFY == cmd) {
                if ((value = touch_get_key_value(split_str[i], "condition_frame_limit")) >= 0) {
                ...
                } else if ((value = touch_get_key_value(split_str[i], "condition_updelay_ms")) >=
                           0) {
                    ...
                    if (grip_info->grip_handle_in_fw && op && op->set_large_frame_limit && g_tp &&
                        !g_tp->loading_fw) {
                    ...
                    if (grip_info->grip_handle_in_fw && op && op->set_large_ver_thd && g_tp &&
                        !g_tp->loading_fw) {
                ...
                } else if ((value = touch_get_key_value(split_str[i], "large_hor_corner_thd")) >=
                           0) {
                    grip_info->large_hor_corner_thd = value;
                    TPD_INFO("change large_hor_corner_thd to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i], "large_ver_corner_thd")) >=
                           0) {
                    grip_info->large_ver_corner_thd = value;
                    TPD_INFO("change large_ver_corner_thd to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i],
                ...
                } else if ((value = touch_get_key_value(split_str[i], "large_ver_corner_width")) >=
                           0) {
                    grip_info->large_ver_corner_width = value;
                    TPD_INFO("change large_ver_corner_width to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i], "large_hor_corner_width")) >=
                           0) {
                    grip_info->large_hor_corner_width = value;
                    TPD_INFO("change large_hor_corner_width to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i], "large_corner_distance")) >=
                           0) {
                ...
                } else if ((value = touch_get_key_value(split_str[i], "large_detect_time_ms")) >=
                           0) {
            ...
            if (OPERATE_MODIFY == cmd) {
                ...
                } else if ((value = touch_get_key_value(split_str[i], "normal_finger_thd_1")) >=
                           0) {
                    long_side_para->normal_finger_thd_1 = value;
                    TPD_INFO("change long side normal_finger_thd_1 to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i], "normal_finger_thd_2")) >=
                           0) {
                    long_side_para->normal_finger_thd_2 = value;
                    TPD_INFO("change long side normal_finger_thd_2 to %d.\n", value);
                } else if ((value = touch_get_key_value(split_str[i], "normal_finger_thd_3")) >=
                           0) {
...
static ssize_t kernel_grip_write(struct file* file, const char __user* buffer, size_t count,
                                 loff_t* ppos) {
    char buf[PAGESIZE] = {0};
    struct kernel_grip_info* grip_info = PDE_DATA(file_inode(file));

    if (!grip_info) return count;
...
static ssize_t proc_touch_dir_read(struct file* file, char __user* user_buf, size_t count,
                                   loff_t* ppos) {
    ssize_t ret = 0;
...
static ssize_t proc_touch_dir_write(struct file* file, const char __user* buffer, size_t count,
                                    loff_t* ppos) {
    char buf[8] = {0};
    int temp = 0;
    struct kernel_grip_info* grip_info = PDE_DATA(file_inode(file));
...
void init_kernel_grip_proc(struct proc_dir_entry* prEntry_tp, struct kernel_grip_info* grip_info) {
    ...
    if (!grip_info || !prEntry_tp) return;

    prEntry_tmp = proc_create_data("kernel_grip_handle", 0666, prEntry_tp, &tp_kernel_grip_fops,
                                   grip_info);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
    ...
    prEntry_tmp = proc_create_data("oplus_tp_direction", 0666, prEntry_tp, &touch_dir_proc_fops,
                                   grip_info);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
...
struct kernel_grip_info* kernel_grip_init(struct device* dev) {
    ...
    int temp_array[2] = {0}, cond_width[4] = {0}, large_width[2] = {0}, large_corner_width[3] = {0},
        eli_width[6] = {0}, curved_large_width[5] = {0};
    struct grip_zone_area* grip_zone = NULL;
    ...
    ret = of_property_read_u32_array(dev->of_node, "prevention,large_corner_judge_para",
                                     large_corner_para, 6);
    if (ret) {
    ...
    if (grip_info->is_curved_screen) {
        ret = of_property_read_u32_array(dev->of_node, "prevention,large_curved_long_judge_para",
                                         long_judge_para, 11);
        if (ret) {
            long_judge_para[0] = 7;
        ...
        ret = of_property_read_u32_array(dev->of_node, "prevention,large_curved_short_judge_para",
                                         short_judge_para, 11);
        if (ret) {
        ...
        ret = of_property_read_u32_array(dev->of_node, "prevention,curved_large_area_width",
                                         curved_large_width, 5);
        if (ret) {
        ...
        ret = of_property_read_u32_array(dev->of_node, "prevention,grip_large_detect_time",
                                         temp_array, 1);
        if (ret) {
        ...
        ret = of_property_read_u32_array(dev->of_node, "prevention,grip_down_delta_time",
                                         temp_array, 1);
        if (ret) {
    ...
    ret = of_property_read_u32_array(dev->of_node, "prevention,condition_area_width", cond_width,
                                     4);
    if (ret) {
        cond_width[0] = 30;
        cond_width[1] = 30;
    ...
    grip_info->grip_handle_in_fw =
            of_property_read_bool(dev->of_node, "prevention,grip_handle_in_fw");
    if (grip_info->grip_handle_in_fw) {
    ...
    if (grip_zone) {
        ...
        grip_zone->support_dir =
                (1 << VERTICAL_SCREEN) | (1 << LANDSCAPE_SCREEN_90) | (1 << LANDSCAPE_SCREEN_270);
    ...
    grip_info->coord_buf = kzalloc(
            TOUCH_MAX_NUM * grip_info->coord_filter_cnt * sizeof(struct coord_buffer), GFP_KERNEL);
    if (grip_info->coord_buf) {
        for (i = 0; i < TOUCH_MAX_NUM; i++) {
            for (j = 0; j < grip_info->coord_filter_cnt; j++) {
                grip_info->coord_buf[i * grip_info->coord_filter_cnt + j].weight =
                        makeup_para[j + 1];
            }
        }
    } else {
