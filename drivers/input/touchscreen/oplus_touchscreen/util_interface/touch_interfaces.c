...
int touch_i2c_read_block(struct i2c_client* client, u16 addr, unsigned short length,
                         unsigned char* data) {
    int retval;
    unsigned char retry;
 ...
 */
#ifndef CONFIG_HAVE_ARCH_VMAP_STACK
int touch_i2c_write_block(struct i2c_client* client, u16 addr, unsigned short length,
...
int touch_i2c_write_byte(struct i2c_client* client, unsigned short addr, unsigned char data) {
    ...
    if (retval == length_trans) retval = 0;
...
int touch_i2c_read_word(struct i2c_client* client, unsigned short addr) {
    ...
    if (retval >= 0) retval = buf[1] << 8 | buf[0];
 ...
 */
int touch_i2c_read(struct i2c_client* client, char* writebuf, int writelen, char* readbuf,
                   int readlen) {
    int retval = 0;
    int retry = 0;
 ...
 * @flag_register_16bit: bool param to detect whether this device using 16bit IIC address or 8bit
 * address
 *
 * Actully, This function don't have many operation, we just detect device address length && alloc
 * DMA memory for MTK platform Returning zero(sucess) or -ENOMEM(memory alloc failed)
 */
...
int32_t spi_read_write(struct spi_device* client, uint8_t* buf, size_t len, uint8_t* rbuf,
                       SPI_RW rw) {
