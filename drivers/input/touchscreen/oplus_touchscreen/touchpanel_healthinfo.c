...
int add_point_to_record(struct points_record* points_record, struct point_info point) {
    ...
    int length = points_record->count < RECORD_POINTS_COUNT - 1 ? points_record->count
                                                                : RECORD_POINTS_COUNT - 1;
    for (i = length; i > 0; i--) {
...
int add_swipe_to_record(struct swipes_record* swipes_record, struct point_info start_point,
                        struct point_info end_point) {
    int i = 0;
    // int length = sizeof(swipes_record->start_points) / sizeof(struct Coordinate);
    int length = swipes_record->count < RECORD_POINTS_COUNT - 1 ? swipes_record->count
                                                                : RECORD_POINTS_COUNT - 1;
...
int print_point_from_record(struct seq_file* s, struct points_record* points_record, char* prefix) {
    int i = 0;
    int length =
            points_record->count < RECORD_POINTS_COUNT ? points_record->count : RECORD_POINTS_COUNT;
    ...
    for (i = 0; i < length; i++) {
        ...
        TPD_DETAIL("%spoint[%d]:[%d %d]\n", prefix ? prefix : "", i, points_record->points[i].x,
                   points_record->points[i].y);
    }
...
int print_swipe_from_record(struct seq_file* s, struct swipes_record* swipes_record, char* prefix) {
    int i = 0;
    int length =
            swipes_record->count < RECORD_POINTS_COUNT ? swipes_record->count : RECORD_POINTS_COUNT;
...
int print_value_count_list(struct seq_file* s, struct list_head* list, value_record_type value_type,
                           char* prefix) {
    struct list_head* pos = NULL;
    struct health_value_count* vc = NULL;
    int* vc_value = NULL;
    ...
    list_for_each(pos, list) {
        ...
        if (!list_num) {
            ...
            if (vc->value) {
                ...
                vc->count = recordlen;
                TPD_DETAIL("len = %d\n", vc->count);
                feedback = print_as_matrix(NULL, vc->value, vc->count, DEFAULT_BUF_MATRIX_LINEBREAK,
                                           true);
            } else {
                TPD_INFO("vc->value kzalloc failed.\n");
                vc->value = next_str;
...
void print_buffer_list(struct seq_file* s, struct list_head* list, char* prefix) {
    ...
    list_for_each(pos, list) {
        ...
        if (s) {
            seq_printf(s, "%s%d:len=%d, ", prefix ? prefix : "", err_num++, vc->count);
        }
...
int point_state_up_handle(struct monitor_data_v2* monitor_data, int i, int direction) {
    ...
    if (monitor_data->points_state[i].down_up_state == STATE_DOWN) {
        ...
        if (monitor_data->points_state[i].max_swipe_distance_sq == 0) {
            ...
            monitor_data
                    ->click_count_array[monitor_data->points_state[i].last_point.y *
                                                CLICK_COUNT_ARRAY_HEIGHT / monitor_data->max_y *
                                                CLICK_COUNT_ARRAY_WIDTH +
                                        monitor_data->points_state[i].last_point.x *
                                                CLICK_COUNT_ARRAY_WIDTH / monitor_data->max_x]++;
        } else {
...
int tp_touch_healthinfo_handle(struct monitor_data_v2* monitor_data, int obj_attention,
                               struct point_info* points, int finger_num, int max_num,
                               int direction) {
    int i = 0, j = 0;
            ...
            if (monitor_data->points_state[i].down_up_state == STATE_DOWN &&
                ...
                if (monitor_data->points_state[i].down_up_state == STATE_UP) {
                    ...
                    if (monitor_data->points_state[i].touch_action == ACTION_SWIPE &&
                        ...
                        !check_healthinfo_time_counter_timeout(
                                ...
                                monitor_data->in_game_mode
                                        ? (SWIPE_BROKEN_FRAMES_MS /
                                           monitor_data->report_rate_in_game)
                                        : (SWIPE_BROKEN_FRAMES_MS /
                                           monitor_data->report_rate))) {  // up and down in short
                                                                           // time(2.5 frames) and
                                                                           // short distance. judge
                                                                           // as broken swipe
                        // add_swipe_to_record(&monitor_data->broken_swipes,
                        // monitor_data->points_state[i].last_point, points[i]);
                        monitor_data->broken_swipes_count_array
                                [points[i].y * (monitor_data->RX_NUM / 2) / monitor_data->max_y *
                    ...
                    if (monitor_data->points_state[i].touch_action == ACTION_CLICK &&
                        ...
                        !check_healthinfo_time_counter_timeout(
                                ...
                                monitor_data->in_game_mode
                                        ...
                                        : (JUMPING_POINT_FRAMES_MS /
                                           monitor_data
                                                   ->report_rate))) {  // time between last click
                                                                       // and this down less
                                                                       // than 1.5 frames, and
                                                                       // distance less than 20px,
                                                                       // judge as jumping point
                ...
                if (!(monitor_data->points_state[i].last_point.x == points[i].x &&
                    ...
                    if (!is_first_point) {
                        ...
                        if (dist_sq > monitor_data->points_state[i].last_swipe_distance_sq &&
                            int_sqrt(dist_sq) - int_sqrt(monitor_data->points_state[i]
                                                                 .last_swipe_distance_sq) >
                                    monitor_data->long_swipe_judge_distance) {  // distance between
                                                                                // points changed
                                                                                // sunddently when
                                                                                // swiping
                            add_swipe_to_record(&monitor_data->long_swipes,
                                                monitor_data->points_state[i].last_point,
                                                points[i]);
                            TPD_DETAIL(
                                    "long swipe:[%d %d] to [%d %d], dis:%d to %d.\n",
                                    monitor_data->points_state[i].last_point.x,
                                    monitor_data->points_state[i].last_point.y, points[i].x,
                                    points[i].y,
                ...
                if (monitor_data->points_state[i].jumping_times > JUMPING_POINT_TIMES) {
                    if (!monitor_data->points_state[i].is_down_handled) {
                        ...
                        monitor_data->jumping_points_count_array
                                [points[i].y * (monitor_data->RX_NUM / 2) / monitor_data->max_y *
                                         (monitor_data->TX_NUM / 2) +
                                 points[i].x * (monitor_data->TX_NUM / 2) / monitor_data->max_x]++;
                        catch_delta_data(
                ...
                if (!monitor_data->points_state[i].is_down_handled &&
                    ...
                    if (direction == VERTICAL_SCREEN) {
                        ...
                        monitor_data->stuck_points_count_array
                                [points[i].y * (monitor_data->RX_NUM / 2) / monitor_data->max_y *
                                         (monitor_data->TX_NUM / 2) +
                                 points[i].x * (monitor_data->TX_NUM / 2) / monitor_data->max_x]++;
                        ...
                        monitor_data->lanscape_stuck_points_count_array
                                [points[i].y * (monitor_data->RX_NUM / 2) / monitor_data->max_y *
                                         (monitor_data->TX_NUM / 2) +
                                 points[i].x * (monitor_data->TX_NUM / 2) / monitor_data->max_x]++;
                    ...
                    catch_delta_data(
                            monitor_data,
                            monitor_data->stuck_point_delta_data);  // coordinate not change over
                                                                    // 15s, catch delta data
                    /*
                    report = kzalloc(DEFAULT_REPORT_STR_LEN, GFP_KERNEL);
                    if (report) {
                        snprintf(report, DEFAULT_REPORT_STR_LEN, "StuckPoint$$Count@@%d$$Coor@@[%d
                    %d]", monitor_data->stuck_points.count, points[i].x, points[i].y);
                ...
                if (!monitor_data->kernel_grip_support) {
                    ...
                    if (!monitor_data->points_state[i].is_down_handled &&
                        ...
                        is_point_in_zone(&monitor_data->dead_zone_list,
                                         monitor_data->points_state[i].first_point,
                                         direction)) {  // grip dead zone 1
                        monitor_data->points_state[i].is_down_handled = true;
                        TPD_DETAIL("swipe from dead zone[%d %d] to [%d %d].\n",
                                   monitor_data->points_state[i].first_point.x,
                                   monitor_data->points_state[i].first_point.y, points[i].x,
                                   points[i].y);
                    } else if (monitor_data->points_state[i].max_swipe_distance_sq == 0 &&
                               !monitor_data->points_state[i].is_down_handled &&
                               is_point_in_zone(&monitor_data->condition_zone_list, points[i],
                                                direction) &&
                               check_healthinfo_time_counter_timeout(
                                       monitor_data->points_state[i].time_counter,
                                       LONG_CLICK_TIME)) {  // grip condition zone long
                                                            // press(>400ms)
            ...
            if (direction == VERTICAL_SCREEN) {
                if ((elizone_points_tophalf_count || elizone_points_bothalf_count) &&
                    ...
                    if (need_record_eli_point_tophalf) {
                        // add_point_to_record(&monitor_data->elimination_zone_points,
                        // points[elizone_point_tophalf_i]); TPD_DETAIL("eli zone [%d %d] / %d.\n",
                        // points[elizone_point_tophalf_i].x, points[elizone_point_tophalf_i].y,
                        // monitor_data->elimination_zone_points.count);
                    ...
                    if (need_record_eli_point_bothalf) {
                        // add_point_to_record(&monitor_data->elimination_zone_points,
                        // points[elizone_point_bothalf_i]); TPD_DETAIL("eli zone [%d %d] / %d.\n",
                        // points[elizone_point_bothalf_i].x, points[elizone_point_bothalf_i].y,
                        // monitor_data->elimination_zone_points.count);
                ...
                if (elizone_points_tophalf_count &&
                    ...
                    need_record_eli_point_tophalf) {
                    // add_point_to_record(&monitor_data->lanscape_elimination_zone_points,
                    // points[elizone_point_tophalf_i]); TPD_DETAIL("lans eli zone [%d %d] / %d.\n",
                    // points[elizone_point_tophalf_i].x, points[elizone_point_tophalf_i].y,
                    // monitor_data->lanscape_elimination_zone_points.count);
                    monitor_data->elizone_point_tophalf_i = elizone_point_tophalf_i;
                    monitor_data->points_state[elizone_point_tophalf_i].is_down_handled = true;
                ...
                if (elizone_points_bothalf_count &&
                    ...
                    need_record_eli_point_bothalf) {
                    // add_point_to_record(&monitor_data->lanscape_elimination_zone_points,
                    // points[elizone_point_bothalf_i]); TPD_DETAIL("lans eli zone [%d %d] / %d.\n",
                    // points[elizone_point_bothalf_i].x, points[elizone_point_bothalf_i].y,
                    // monitor_data->lanscape_elimination_zone_points.count);
                    monitor_data->elizone_point_bothalf_i = elizone_point_bothalf_i;
                    monitor_data->points_state[elizone_point_bothalf_i].is_down_handled = true;
...
int tp_grip_up_healthinfo_handle(struct monitor_data_v2* monitor_data, uint8_t up_id,
                                 int direction) {
    int ret = 0;
...
void reset_healthinfo_grip_time_record(void* tp_monitor_data, void* tp_grip_info) {
    ...
    if (monitor_data->grip_time_record_flag == TYPE_START_RECORD) {
        monitor_data->total_grip_time_no_touch +=
                ktime_to_ms(ktime_get()) - monitor_data->grip_start_time_no_touch;
...
int tp_grip_healthinfo_handle(struct monitor_data_v2* monitor_data,
...
int tp_suspend_healthinfo_handle(struct monitor_data_v2* monitor_data, u64 start_time) {
    ...
    screenon_time = monitor_data->screenon_timer
                            ? check_healthinfo_time_counter_timeout(monitor_data->screenon_timer, 0)
                            : 0;
...
int tp_test_healthinfo_handle(struct monitor_data_v2* monitor_data, healthinfo_type test_type,
                              int error_count) {
    int ret = 0;
...
int tp_bus_err_healthinfo_handle(struct monitor_data_v2* monitor_data, int errno,
    ...
    buff_part = record_buffer_data(&monitor_data->bus_errs_buff_list, writebuf, writelen,
                                   MAX_BUS_ERROR_BUFF_RECORD);
...
int tp_alloc_healthinfo_handle(struct monitor_data_v2* monitor_data, long alloc_size,
                               bool alloc_success) {
    int ret = 0;
    int deep = 2;
    char* report = NULL;
    ...
    if (alloc_success) {
        ...
        while (deep) {
#if IS_BUILTIN(CONFIG_TOUCHSCREEN_OPLUS)
    ...
    switch (type) {
        case HEALTH_TOUCH:
            if (monitor_data->touch_points) {
                ret = tp_touch_healthinfo_handle(
                        monitor_data, *value_int, monitor_data->touch_points,
                        monitor_data->touch_num, monitor_data->max_finger_support,
        ...
        case HEALTH_BUS:
            if (monitor_data->bus_buf) {
                ret = tp_bus_err_healthinfo_handle(monitor_data, *value_int, monitor_data->bus_buf,
                                                   monitor_data->bus_len);
...
int tp_healthinfo_read(struct seq_file* s, void* tp_monitor_data) {
    ...
    if (monitor_data->max_touch_num_in_game) {
        seq_printf(s,
                   "total_touch_time(in_game):%lld[%lld %lld %lld %lld %lld %lld %lld %lld %lld "
                   "%lld]s\n",
    ...
    if (monitor_data->max_jumping_times > JUMPING_POINT_TIMES) {
        ...
        if (!is_delta_data_allzero(monitor_data->jumping_points_count_array,
            ...
            print_delta_data(s, monitor_data->jumping_points_count_array, monitor_data->RX_NUM / 2,
                             monitor_data->TX_NUM / 2);
        }
        if (!is_delta_data_allzero(monitor_data->jumping_point_delta_data, monitor_data->TX_NUM,
                                   monitor_data->RX_NUM)) {
            print_delta_data(s, monitor_data->jumping_point_delta_data, monitor_data->TX_NUM,
                             monitor_data->RX_NUM);
        }
    }
    ...
    if (!is_delta_data_allzero(monitor_data->stuck_points_count_array, monitor_data->RX_NUM / 2,
                               monitor_data->TX_NUM / 2)) {
    ...
    if (!is_delta_data_allzero(monitor_data->stuck_point_delta_data, monitor_data->TX_NUM,
                               monitor_data->RX_NUM)) {
    ...
    if (!is_delta_data_allzero(monitor_data->broken_swipes_count_array, monitor_data->RX_NUM / 2,
                               monitor_data->TX_NUM / 2)) {
        seq_printf(s, "%sswipe:", PREFIX_SWIPE_BROKER);
        print_delta_data(s, monitor_data->broken_swipes_count_array, monitor_data->RX_NUM / 2,
                         monitor_data->TX_NUM / 2);
    }
...
int tp_healthinfo_init(struct device* dev, void* tp_monitor_data) {
    ...
    monitor_data->long_swipe_judge_distance = int_sqrt(monitor_data->max_x * monitor_data->max_x +
                                                       monitor_data->max_y * monitor_data->max_y) /
                                              LONG_SWIPE_JUDGE_RATIO;
    monitor_data->swipe_broken_judge_distance =
            int_sqrt(monitor_data->max_x * monitor_data->max_x +
                     monitor_data->max_y * monitor_data->max_y) /
    ...
    TPD_INFO(
            "long_swipe_judge_distance=%d swipe_broken_judge_distance=%d "
            "jumping_point_judge_distance=%d\n",
            monitor_data->long_swipe_judge_distance, monitor_data->swipe_broken_judge_distance,
            monitor_data->jumping_point_judge_distance);
    ...
    monitor_data->jumping_point_delta_data =
            kzalloc(sizeof(int32_t) * monitor_data->TX_NUM * monitor_data->RX_NUM, GFP_KERNEL);
    ...
    monitor_data->stuck_point_delta_data =
            kzalloc(sizeof(int32_t) * monitor_data->TX_NUM * monitor_data->RX_NUM, GFP_KERNEL);
    ...
    monitor_data->points_state = kzalloc(
            sizeof(struct point_state_monitor) * monitor_data->max_finger_support, GFP_KERNEL);
    if (!monitor_data->points_state) {
    ...
    monitor_data->click_count_array = kzalloc(
            sizeof(int32_t) * CLICK_COUNT_ARRAY_HEIGHT * CLICK_COUNT_ARRAY_WIDTH, GFP_KERNEL);
    ...
    monitor_data->jumping_points_count_array = kzalloc(
            sizeof(int32_t) * (monitor_data->TX_NUM / 2) * (monitor_data->RX_NUM / 2), GFP_KERNEL);
    ...
    monitor_data->stuck_points_count_array = kzalloc(
            sizeof(int32_t) * (monitor_data->TX_NUM / 2) * (monitor_data->RX_NUM / 2), GFP_KERNEL);
    ...
    monitor_data->lanscape_stuck_points_count_array = kzalloc(
            sizeof(int32_t) * (monitor_data->TX_NUM / 2) * (monitor_data->RX_NUM / 2), GFP_KERNEL);
    ...
    monitor_data->broken_swipes_count_array = kzalloc(
            sizeof(int32_t) * (monitor_data->TX_NUM / 2) * (monitor_data->RX_NUM / 2), GFP_KERNEL);
    ...
    monitor_data->total_touch_time_in_game =
            kzalloc(sizeof(u64) * (monitor_data->max_finger_support + 1), GFP_KERNEL);
