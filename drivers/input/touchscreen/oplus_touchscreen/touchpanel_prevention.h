...
struct grip_zone_area {
    struct list_head
            area_list;  // must be the first member to easy get the pointer of grip_zone_area
    uint16_t start_x;
    uint16_t start_y;
    uint16_t x_width;
    uint16_t y_width;
...
struct kernel_grip_info {
    ...
    uint8_t large_point_status[TOUCH_MAX_NUM];  /*parameter for judge if ponits in large area need
                                                   make up or not*/
    ...
    uint16_t large_ver_corner_width;   /*threshold to determine should be judged in vertical corner
                                          way*/
    uint16_t large_hor_corner_width;   /*threshold to determine should be judged in horizon corner
                                          way*/
