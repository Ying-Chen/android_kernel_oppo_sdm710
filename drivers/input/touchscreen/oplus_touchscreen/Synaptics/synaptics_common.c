...
int synaptics_get_limit_data(char* type, const unsigned char* fw_image) {
    ...
    for (i = 0; i < count; i++) {
        ...
        offset += (sizeof(struct limit_block) - 4 +
...
int synaptics_parse_header_v2(struct image_info* image_info, const unsigned char* fw_image) {
    ...
    for (i = 0; i < number_of_areas; i++) {
        ...
        if (magic_value != FLASH_AREA_MAGIC_VALUE) continue;
        ...
        if (0 == strncmp((char*)descriptor->id_string, BOOT_CONFIG_ID, strlen(BOOT_CONFIG_ID))) {
        ...
        } else if (0 == strncmp((char*)descriptor->id_string, APP_CODE_ID, strlen(APP_CODE_ID))) {
            if (checksum != (crc32(~0, content, length) ^ ~0)) {
            ...
            pr_info("Application firmware size = %d address = 0x%08x\n", length, flash_addr);
                   ...
                   strncmp((char*)descriptor->id_string, APP_CONFIG_ID, strlen(APP_CONFIG_ID))) {
            if (checksum != (crc32(~0, content, length) ^ ~0)) {
            ...
            pr_info("Application config size = %d address = 0x%08x\n", length, flash_addr);
                   ...
                   strncmp((char*)descriptor->id_string, DISP_CONFIG_ID, strlen(DISP_CONFIG_ID))) {
            if (checksum != (crc32(~0, content, length) ^ ~0)) {
...
void synaptics_parse_header(struct image_header_data* header, const unsigned char* fw_image) {
    ...
    if (header->contains_firmware_id) header->firmware_id = extract_uint_le(data->firmware_id);
...
void synaptics_print_limit_v3(struct seq_file* s, struct touchpanel_data* ts,
    ...
    for (m = 0; m < item_cnt; m++) {
        ...
        seq_printf(s, "item %d[size %d, limit type %d, para num %d] :\n", item_head->item_bit,
                   item_head->item_size, item_head->item_limit_type, item_head->para_num);
...
static ssize_t proc_touchfilter_control_read(struct file* file, char __user* user_buf, size_t count,
                                             loff_t* ppos) {
    ssize_t ret = 0;
    ...
    if (!syn_ops->get_touchfilter_state) return 0;
...
static ssize_t proc_touchfilter_control_write(struct file* file, const char __user* buffer,
    ...
    if (!syn_ops->set_touchfilter_state) return count;
...
int synaptics_create_proc(struct touchpanel_data* ts, struct synaptics_proc_operations* syna_ops) {
    ...
    prEntry_tmp =
            proc_create_data("baseline_test", 0666, ts->prEntry_tp, &tp_auto_test_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
    ...
    if (ts->face_detect_support) {
        prEntry_tmp =
                proc_create_data("touch_filter", 0666, ts->prEntry_tp, &touch_filter_proc_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
