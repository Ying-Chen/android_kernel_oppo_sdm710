...
void gf_cleanup_pwr_list(struct gf_dev* gf_dev) {
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        pr_err("gf_cleanup_pwr_list vdd_flag = %d!\n",vdd_flag);
        if (vdd_flag == 1) {
            gpio_free(gf_dev->vdd_gpio);
            pr_err("remove vdd_gpio success\n");
        }
        pr_err("---- free pwr ok  with mode = %d, index = %d  ----\n",
...
int gf_parse_pwr_list(struct gf_dev* gf_dev) {
    ...
    for_each_available_child_of_node(np, child) {
        ...
        case FP_POWER_MODE_GPIO:
            ...
            ret = of_property_read_u32(np, VDD_FLAG, &vdd_flag);
            pr_err("vdd_flag = %d!\n",vdd_flag);
            if (vdd_flag == 1) {
                ...
                ret = devm_gpio_request(dev, gf_dev->vdd_gpio, "goodix_vdd");
                if (ret) {
                    pr_err("failed to request goodix_vdd gpio, ret = %d\n", ret);
...
int gf_power_on(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        case FP_POWER_MODE_GPIO:
            ...
            ret = of_property_read_u32(np, VDD_FLAG, &vdd_flag);
            pr_err("gf_power_on vdd_flag = %d!\n",vdd_flag);
            if(vdd_flag == 1) {
                msleep(5);
                gpio_set_value(gf_dev->vdd_gpio, 1);
                pr_err("set pwe_gpio 1 for 19651 \n");                
            }
            break;
...
int gf_power_off(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
        ...
        case FP_POWER_MODE_GPIO:
            ...
            ret = of_property_read_u32(np, VDD_FLAG, &vdd_flag);
            pr_err("gf_power_off  vdd_flag = %d!\n",vdd_flag);
            if (vdd_flag == 1) {
                gpio_set_value(gf_dev->vdd_gpio, 0);
                pr_err("poweroff set pwe_gpio 1 for 19651 \n");
            }
            break;
        case FP_POWER_MODE_AUTO:
            pr_info("[%s] power on auto, no need power on again\n", __func__);
