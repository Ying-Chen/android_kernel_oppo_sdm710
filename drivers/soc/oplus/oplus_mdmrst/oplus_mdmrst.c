...
static ssize_t mdmreason_write(struct file* file, const char __user* buf, size_t count,
                               loff_t* ppos) {
    /*just for test*/
...
static ssize_t mdmreason_read(struct file* file, char __user* buf, size_t count, loff_t* ppos) {
    size_t size = 0;
...
ssize_t mdmrst_read(struct file* file, char __user* buf, size_t count, loff_t* ppos) {
    char* tmp = "1";
...
static struct miscdevice mdmrst_device = {MISC_DYNAMIC_MINOR, "mdmrst", &mdmrst_device_fops};
