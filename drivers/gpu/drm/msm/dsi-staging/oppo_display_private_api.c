...
static ssize_t oppo_display_set_hbm(struct device *dev,
	...
	sscanf(buf, "%du", &temp_save);
	printk(KERN_ERR "%s oppo_display_set_hbm = %d\n", __func__, temp_save);
	if (get_oppo_display_power_status() != OPPO_DISPLAY_POWER_ON) {
...
int oppo_get_panel_brightness_to_alpha(void)
{
	...
	if (hbm_mode) {
		pr_err("%s: hbm_mode=%d, direct return!\n", __func__, hbm_mode);
		return 0;
	}

	if (!oppo_ffl_trigger_finish) {
		pr_err("%s: ffl trigger no finish yet!\n", __func__);
                               ...
                               const char *buf, size_t count)
{
	...
	pr_err("%s oppo_panel_alpha=%d\n", __func__, oppo_panel_alpha);
	return count;
}
                               ...
                               const char *buf, size_t count)
{
	...
	pr_err("%s: oppo_dimlayer_hbm = %d\n", __func__, oppo_dimlayer_hbm);
		...
		const char *buf, size_t count)
{
	...
	pr_err("%s notify fingerpress %s\n", __func__, onscreenfp_status ? "on" : "off");
...
static DEVICE_ATTR(LCM_CABC, S_IRUGO|S_IWUSR, oppo_display_get_cabc, oppo_display_set_cabc);
