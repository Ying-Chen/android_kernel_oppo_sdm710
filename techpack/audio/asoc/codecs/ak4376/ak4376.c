...
static unsigned int ak4376_reg_read(struct snd_soc_codec* codec, unsigned int reg);
...
static int ak4376_update_register(struct snd_soc_codec* codec) {
    ...
    if (ak4376->deviceID == -1) return 0;

    if (ak4376->deviceID == 0x22) {
        for (i = 0; i < ARRAY_SIZE(ak4377a_patch); i++) {
            snd_soc_write(codec, ak4377a_patch[i].reg, ak4377a_patch[i].def);
        }
    } else {
        for (i = 0; i < ARRAY_SIZE(ak4376a_patch); i++) {
            snd_soc_write(codec, ak4376a_patch[i].reg, ak4376a_patch[i].def);
        }
    }
...
static ssize_t reg_data_store(struct device* dev, struct device_attribute* attr, const char* buf,
                              size_t count) {
    ...
    for (i = 0; i < pt_count; i += 2) {
        snd_soc_write(ak4376_codec, (unsigned int)val[i], (unsigned int)val[i + 1]);
...
static const char* ak4376_cpmod_select_texts[] = {"Automatic Switching", "+-VDD Operation",
                                                  "+-1/2VDD Operation"};
static const char* ak4376_hphl_select_texts[] = {"9ohm", "Hi-Z"};
static const char* ak4376_hphr_select_texts[] = {"9ohm", "Hi-Z"};
static const char* ak4376_dacfil_select_texts[] = {"Sharp Roll-Off", "Slow Roll-Off",
...
static const struct soc_enum ak4376_dac_enum[] = {
        SOC_ENUM_SINGLE(AK4376_0B_LCH_OUTPUT_VOLUME, 7, ARRAY_SIZE(ak4376_ovolcn_select_texts),
                        ak4376_ovolcn_select_texts),
        SOC_ENUM_SINGLE(AK4376_07_DAC_MONO_MIXING, 2, ARRAY_SIZE(ak4376_mdacl_select_texts),
                        ak4376_mdacl_select_texts),
        SOC_ENUM_SINGLE(AK4376_07_DAC_MONO_MIXING, 6, ARRAY_SIZE(ak4376_mdacr_select_texts),
                        ak4376_mdacr_select_texts),
        SOC_ENUM_SINGLE(AK4376_07_DAC_MONO_MIXING, 3, ARRAY_SIZE(ak4376_invl_select_texts),
                        ak4376_invl_select_texts),
        SOC_ENUM_SINGLE(AK4376_07_DAC_MONO_MIXING, 7, ARRAY_SIZE(ak4376_invr_select_texts),
                        ak4376_invr_select_texts),
        SOC_ENUM_SINGLE(AK4376_03_POWER_MANAGEMENT4, 2, ARRAY_SIZE(ak4376_cpmod_select_texts),
                        ak4376_cpmod_select_texts),
        SOC_ENUM_SINGLE(AK4376_04_OUTPUT_MODE_SETTING, 0, ARRAY_SIZE(ak4376_hphl_select_texts),
                        ak4376_hphl_select_texts),
        SOC_ENUM_SINGLE(AK4376_04_OUTPUT_MODE_SETTING, 1, ARRAY_SIZE(ak4376_hphr_select_texts),
                        ak4376_hphr_select_texts),
        SOC_ENUM_SINGLE(AK4376_06_DIGITAL_FILTER_SELECT, 6, ARRAY_SIZE(ak4376_dacfil_select_texts),
                        ak4376_dacfil_select_texts),
        SOC_ENUM_SINGLE(AK4376_15_AUDIO_IF_FORMAT, 3, ARRAY_SIZE(ak4376_bcko_select_texts),
                        ak4376_bcko_select_texts),
        SOC_ENUM_SINGLE(AK4376_06_DIGITAL_FILTER_SELECT, 3, ARRAY_SIZE(ak4376_dfthr_select_texts),
                        ak4376_dfthr_select_texts),
        SOC_ENUM_SINGLE(AK4376_06_DIGITAL_FILTER_SELECT, 0, ARRAY_SIZE(ak4376_ngate_select_texts),
                        ak4376_ngate_select_texts),
        SOC_ENUM_SINGLE(AK4376_06_DIGITAL_FILTER_SELECT, 1, ARRAY_SIZE(ak4376_ngatet_select_texts),
...
static int ak4376_set_bickfs(struct snd_soc_codec* codec) {
    ...
    if (ak4376->nBickFreq == 0) {  // 32fs
        snd_soc_update_bits(codec, AK4376_15_AUDIO_IF_FORMAT, 0x03,
...
static int ak4376_set_lpmode(struct snd_soc_codec* codec) {
    ...
    if (ak4376->lpmode == 0) {  // High Performance Mode
        snd_soc_update_bits(codec, AK4376_02_POWER_MANAGEMENT3, 0x10,
...
static int ftm_hp_rev_get(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct snd_soc_codec* codec = snd_soc_kcontrol_codec(kcontrol);
    struct ak4376_priv* ak4376 = snd_soc_codec_get_drvdata(codec);
    int retries = 50;
    bool pdn = false;
...
static int ftm_hp_rev_put(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    return 0;
}
...
static const struct snd_kcontrol_new ak4376_snd_controls[] = {
        SOC_SINGLE_TLV("AK4376 Digital Output VolumeL", AK4376_0B_LCH_OUTPUT_VOLUME, 0, 0x1F, 0,
                       ovl_tlv),
        SOC_SINGLE_TLV("AK4376 Digital Output VolumeR", AK4376_0C_RCH_OUTPUT_VOLUME, 0, 0x1F, 0,
                       ovr_tlv),
        SOC_SINGLE_TLV("AK4376 HP-Amp Analog Volume", AK4376_0D_HP_VOLUME_CONTROL, 0, 0x0F, 0,
                       hpg_tlv),
        ...
        SOC_ENUM_EXT("HP_Pa Revision", ftm_hp_rev_enum, ftm_hp_rev_get, ftm_hp_rev_put),
};
        ...
        case SND_SOC_DAPM_POST_PMD: /* after widget power down */
            ...
            if (ak4376->nPllMode == 0) {
                if (MSmode & 0x10) {  // Master mode
                    snd_soc_update_bits(codec, AK4376_15_AUDIO_IF_FORMAT, 0x10, 0x00);  // MS bit =
                                                                                        // 0
                }
            }
...
static int ak4376_dac_event(struct snd_soc_dapm_widget* w, struct snd_kcontrol* kcontrol,
...
static int ak4376_pll_event(struct snd_soc_dapm_widget* w, struct snd_kcontrol* kcontrol,
                            int event)  // CONFIG_LINF
{
    struct snd_soc_codec* codec = snd_soc_dapm_to_codec(w->dapm);
...
static const struct snd_soc_dapm_widget ak4376_dapm_widgets[] = {
        ...
        SND_SOC_DAPM_DAC_E("AK4376 DAC", "NULL", AK4376_02_POWER_MANAGEMENT3, 0, 0,
                           ak4376_dac_event,
                           (SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_PRE_PMU |
                            SND_SOC_DAPM_POST_PMD)),

        // PLL, OSC
        SND_SOC_DAPM_SUPPLY_S("AK4376 PLL", 0, SND_SOC_NOPM, 0, 0, ak4376_pll_event,
                              (SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_PRE_PMU |
                               SND_SOC_DAPM_POST_PMD)),
...
static const struct snd_soc_dapm_route ak4376_intercon[] = {
        {"AK4376 DAC", NULL, "AK4376 PLL"},          {"AK4376 DAC", NULL, "AK4376 SDTI"},

        {"AK4376 HPL Mixer", "LDACL", "AK4376 DAC"}, {"AK4376 HPL Mixer", "RDACL", "AK4376 DAC"},
        {"AK4376 HPR Mixer", "LDACR", "AK4376 DAC"}, {"AK4376 HPR Mixer", "RDACR", "AK4376 DAC"},

        {"AK4376 HPL", NULL, "AK4376 HPL Mixer"},    {"AK4376 HPR", NULL, "AK4376 HPR Mixer"},
};
...
static int ak4376_hw_params(struct snd_pcm_substream* substream, struct snd_pcm_hw_params* params,
...
static int ak4376_set_dai_sysclk(struct snd_soc_dai* dai, int clk_id, unsigned int freq, int dir) {
    struct snd_soc_codec* codec = dai->codec;
    struct ak4376_priv* ak4376 = snd_soc_codec_get_drvdata(codec);
...
static int ak4376_set_bias_level(struct snd_soc_codec* codec, enum snd_soc_bias_level level) {
    struct snd_soc_dapm_context* dapm = snd_soc_codec_get_dapm(codec);

    pr_info("%s: current bias_level=%d\n", __func__, snd_soc_codec_get_bias_level(codec));
...
#define AK4376_RATES                                                                             \
    (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_11025 | SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_22050 |  \
     SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_44100 | SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_88200 | \
...
static int ak4376_init_reg(struct snd_soc_codec* codec) {
    ...
    switch (ak4376->deviceID) {
        ...
        case 0x20:
            ...
            break;
        case 0x21:
            printk("AK4376A is connecting.\n");
            break;
        case 0x22:
            printk("AK4377A is connecting.\n");
            break;
        case 0x30:
            printk("AK4377 is connecting.\n");
            break;
        case 0x70:
            printk("AK4331 is connecting.\n");
            break;
        default:
            printk("This device is not AK437X\n");
            break;
    }
...
static struct of_device_id ak4376_i2c_dt_ids[] = {{.compatible = "akm,ak4376"}, {}};
...
static int ak4376_i2c_probe(struct i2c_client* i2c, const struct i2c_device_id* id) {
    ...
    ak4376->priv_pdn_en = of_get_named_gpio(i2c->dev.of_node, "ak4376,reset-gpio", 0);
    ...
    ret = snd_soc_register_codec(&i2c->dev, &soc_codec_dev_ak4376, &ak4376_dai[0],
                                 ARRAY_SIZE(ak4376_dai));
    if (ret < 0) {
        kfree(ak4376);
    ...
    pr_info("%s: pdn1=%d\n, ret=%d", __func__, ak4376->pdn1, ret);
