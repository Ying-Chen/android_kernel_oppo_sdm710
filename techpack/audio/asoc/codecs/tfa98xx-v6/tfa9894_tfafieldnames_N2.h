 ...
 *  This file was generated automatically on 05/31/18 at 12:32:13.
 *  Source file: TFA9894_N2A0_I2C_RegisterMap.xlsx
 */
...
typedef enum nxpTfa9894N2BfEnumList {
    ...
    TFA9894N2_BF_TDMDCG =
            0x5703, /*!< Second channel gain in case of stereo using a single coil. (Total gain
                       depending on INPLEV). (In case of mono OR stereo using 2 separate DCDC
                       channel 1 should be disabled using TDMDCE) */
    ...
    TFA9894N2_BF_LVLCLPPWM = 0x6152,  /*!< Set the amount of pwm pulse that may be skipped before
    ...
    TFA9894N2_BF_DCCV = 0x7101,       /*!< Slope compensation current, represents LxF (inductance x
                                         frequency) value  */
    ...
    TFA9894N2_BF_DCTRACK = 0x7160,    /*!< Boost algorithm selection, effective only when
                                         boost_intelligent is set to 1 */
    TFA9894N2_BF_DCENVSEL = 0x7170,   /*!< Selection of data for adaptive boost algorithm, effective
                                         only when boost_intelligent is set to 1 */
    ...
    TFA9894N2_BF_DCTRIPT = 0x72a4,     /*!< Track adaptive boost trip levels, effective only when
                                          boost_intelligent is set to 1 */
    TFA9894N2_BF_DCTRIPHYSTE = 0x72f0, /*!< Enable hysteresis on booster trip levels           */
    TFA9894N2_BF_DCHOLD = 0x7304,      /*!< Hold time for DCDC booster, effective only when
                                          boost_intelligent is set to 1 */
...
#define TFA9894N2_NAMETABLE                                                                        \
    static tfaBfName_t Tfa9894N2DatasheetNames[] = {                                               \
            ...
            {0x5703,                                                                               \
             "TDMDCG"}, /* Second channel gain in case of stereo using a single coil. (Total gain  \
                           depending on INPLEV). (In case of mono OR stereo using 2 separate DCDC  \
                           channel 1 should be disabled using TDMDCE), */                          \
            ...
            {0x6152, "LVLCLPPWM"},  /* Set the amount of pwm pulse that may be skipped before      \
                                       clip-flag is triggered, */                                  \
            {0x7005, "DCVOF"},      /* First Boost Voltage Level                         , */      \
            {0x7065, "DCVOS"},      /* Second Boost Voltage Level                        , */      \
            {0x70c3, "DCMCC"},      /* Max Coil Current                                  , */      \
            {0x7101, "DCCV"},       /* Slope compensation current, represents LxF (inductance x    \
                                       frequency) value , */                                       \
            {0x7120, "DCIE"},       /* Adaptive boost mode                               , */      \
            {0x7130, "DCSR"},       /* Soft ramp up/down                                 , */      \
            {0x7140, "DCDIS"},      /* DCDC on/off                                       , */      \
            {0x7150, "DCPWM"},      /* DCDC PWM only mode                                , */      \
            {0x7160, "DCTRACK"},    /* Boost algorithm selection, effective only when              \
                                       boost_intelligent is set to 1, */                           \
            {0x7170, "DCENVSEL"},   /* Selection of data for adaptive boost algorithm, effective   \
                                       only when boost_intelligent is set to 1, */                 \
            {0x7195, "OVSCTLVL"},   /* Threshold level to activate active overshoot control, */    \
            {0x7204, "DCTRIP"}, /* 1st adaptive boost trip levels, effective only when DCIE is set \
                                   to 1, */                                                        \
            {0x7254, "DCTRIP2"}, /* 2nd adaptive boost trip levels, effective only when DCIE is    \
                                    set to 1, */                                                   \
            {0x72a4, "DCTRIPT"}, /* Track adaptive boost trip levels, effective only when          \
                                    boost_intelligent is set to 1, */                              \
            {0x72f0, "DCTRIPHYSTE"}, /* Enable hysteresis on booster trip levels          , */     \
            {0x7304, "DCHOLD"},      /* Hold time for DCDC booster, effective only when            \
                                        boost_intelligent is set to 1, */                          \
            ...
            {0xa350, "CMTPI"}, /* Start copying all the data from mtp to I2C mtp registers - auto  \
                                  clear, */                                                        \
            {0xa360,                                                                               \
...
#define TFA9894N2_BITNAMETABLE                                                                     \
    static tfaBfName_t Tfa9894N2BitNames[] = {                                                     \
            ...
            {0x470, "disable_auto_sel_refclk"}, /* Automatic PLL reference clock selection for     \
                                                   cold start, */                                  \
            ...
            {0xf0f, "hidden_code"},     /* Hidden code to enable access to hidden register.        \
                                           (0x5A6B/23147 default for engineering), */              \
            {0x1000, "flag_por"},       /* POR                                               , */  \
            ...
            {0x5390, "icomp_engage_overrule"}, /* To overrule the functional icomp_engage signal   \
                                                  during validation, */                            \
            ...
            {0x550a, "enbl_amp"}, /* Switch on the class-D power sections, each part of the analog \
            ...
            {0x5703, "ctrl_att_dcdc"}, /* Second channel gain in case of stereo using a single     \
                                          coil. (Total gain depending on INPLEV). (In case of mono \
                                          OR stereo using 2 separate DCDC channel 1 should be      \
                                          disabled using TDMDCE), */                               \
            ...
            {0x5b65, "lpm1_hold_time"}, /* Low power mode hold time before entering into low power \
                                           mode, */                                                \
            ...
            {0x5e02, "rst_min_vbat_delay"}, /* Delay for reseting the min_vbat value inside HW     \
                                               Clipper (number of Fs pulses), */                   \
            {0x5e30,                                                                               \
             "rst_min_vbat_sel"},  /* Control for selecting reset signal for min_bat    , */       \
            {0x5f00, "hard_mute"}, /* Hard mute - PWM                                   , */       \
            {0x5f12, "ns_hp2ln_criterion"}, /* 0..7 zeroes at ns as threshold to swap from         \
                                               high_power to low_noise, */                         \
            {0x5f42, "ns_ln2hp_criterion"}, /* 0..7 zeroes at ns as threshold to swap from         \
                                               low_noise to high_power, */                         \
            {0x5f78, "spare_out"}, /* Spare out register                                , */       \
            {0x600f, "spare_in"},  /* Spare IN                                          , */       \
            {0x6102, "cursense_comp_delay"}, /* Delay to allign compensation signal with current   \
                                                sense signal, */                                   \
            {0x6130,                                                                               \
             "cursense_comp_sign"}, /* Polarity of compensation for current sense        , */      \
            {0x6140,                                                                               \
             "enbl_cursense_comp"},    /* Enable current sense compensation                 , */   \
            {0x6152, "pwms_clip_lvl"}, /* Set the amount of pwm pulse that may be skipped before   \
                                          clip-flag is triggered, */                               \
            {0x7005,                                                                               \
            ...
            {0x7101, "bst_slpcmplvl"}, /* Slope compensation current, represents LxF (inductance x \
                                          frequency) value , */                                    \
            ...
            {0x7160, "boost_track"},   /* Boost algorithm selection, effective only when           \
                                          boost_intelligent is set to 1, */                        \
            {0x7170,                                                                               \
            ...
            {0x7195, "overshoot_correction_lvl"}, /* Threshold level to activate active overshoot  \
                                                     control, */                                   \
            {0x7204, "boost_trip_lvl_1st"}, /* 1st adaptive boost trip levels, effective only when \
                                               DCIE is set to 1, */                                \
            {0x7254, "boost_trip_lvl_2nd"}, /* 2nd adaptive boost trip levels, effective only when \
                                               DCIE is set to 1, */                                \
            {0x72a4, "boost_trip_lvl_track"}, /* Track adaptive boost trip levels, effective only  \
                                                 when boost_intelligent is set to 1, */            \
            {0x72f0, "enbl_trip_hyst"},  /* Enable hysteresis on booster trip levels          , */ \
            {0x7304, "boost_hold_time"}, /* Hold time for DCDC booster, effective only when        \
                                            boost_intelligent is set to 1, */                      \
            {0x7350, "dcdc_pfm20khz_limit"}, /* DCDC in PFM mode pwm mode is activated each 50us   \
                                                to force a pwm pulse, */                           \
            {0x7361, "dcdc_ctrl_maxzercnt"}, /* Number of zero current flags to count before going \
                                                to pfm mode, */                                    \
            {0x7386,                                                                               \
            ...
            {0x7660, "bst_use_new_zercur_detect"}, /* Enable new zero current detection for boost  \
                                                      control, */                                  \
            ...
            {0x8105, "cs_ktemp"},     /* Current sense temperature compensation trimming (1 -      \
                                         VALUE*TEMP) * signal, */                                  \
            ...
            {0x82a0, "cs_adc_slowdel"},  /* Select delay for current sense ADC (internal decision  \
                                            circuitry), */                                         \
            ...
            {0x8310, "cs_classd_tran_skip"}, /* Skip current sense connection during a classD      \
            ...
            {0x8350, "cs_ldo_pulldown"}, /* Pull down current sense LDO, only valid if enbl_cs_ldo \
                                            is high, */                                            \
            {0x8364,                                                                               \
            ...
            {0x8820, "enbl_dc_filter"}, /* Control for enabling the DC blocking filter for voltage \
                                           and current sense, */                                   \
            {0x8901,                                                                               \
            ...
            {0x8960, "vs_adc_slowdel"},  /* Select delay for voltage sense ADC (internal decision  \
                                            circuitry), */                                         \
            {0x8970, "vs_classd_tran_skip"}, /* Skip voltage sense connection during a classD      \
                                                amplifier transition, */                           \
            ...
            {0x8a30, "vs_ldo_pulldown"}, /* Pull down voltage sense LDO, only valid if enbl_cs_ldo \
                                            is high, */                                            \
            {0x8a44,                                                                               \
            ...
            {0xa007, "mtpkey1"}, /* KEY1 To access KEY1 protected registers 0x5A/90d  (default for \
                                    engineering), */                                               \
            ...
            {0xa330, "man_copy_mtp_to_iic"}, /* Start copying single word from MTP to I2C mtp      \
                                                register - auto clear, */                          \
            {0xa340, "man_copy_iic_to_mtp"}, /* Start copying single word from I2C mtp register to \
                                                mtp - auto clear, */                               \
            {0xa350, "auto_copy_mtp_to_iic"}, /* Start copying all the data from mtp to I2C mtp    \
                                                 registers - auto clear, */                        \
            ...
            {0xca05, "pll_inseli"},   /* PLL INSELI - PLL direct bandwidth control mode only with  \
                                         pll_bandsel set to 1, */                                  \
            {0xca64, "pll_inselp"},   /* PLL INSELP - PLL direct bandwidth control mode only with  \
                                         pll_bandsel set to 1, */                                  \
            {0xcab3, "pll_inselr"},   /* PLL INSELR - PLL direct bandwidth control mode only with  \
                                         pll_bandsel set to 1, */                                  \
            {0xcaf0, "pll_bandsel"},  /* PLL bandwidth selection control, USE WITH CAUTION , */    \
            {0xcb09, "pll_ndec"}, /* PLL NDEC in direct control mode only, use_direct_pll_ctrl set \
                                     to 1, */                                                      \
            {0xcba0, "pll_mdec_msb"}, /* MSB of PLL MDEC in direct control mode only,              \
                                         use_direct_pll_ctrl set to 1, */                          \
            {0xcbb0, "pll_bypass"},   /* PLL bypass control during functional mode         , */    \
            {0xcbc0, "pll_directi"},  /* PLL directi control in direct control mode only,          \
                                         use_direct_pll_ctrl set to 1, */                          \
            {0xcbd0, "pll_directo"},  /* PLL directo control in direct control mode only,          \
                                         use_direct_pll_ctrl set to 1, */                          \
            {0xcbe0, "pll_frm_clockstable"}, /* PLL FRM clock stable control in direct control     \
                                                mode only, use_direct_pll_ctrl set to 1, */        \
            {0xcbf0, "pll_frm"},      /* PLL free running mode control in functional mode  , */    \
            {0xcc0f, "pll_mdec_lsb"}, /* Bits 15..0 of PLL MDEC in direct control mode only,       \
                                         use_direct_pll_ctrl set to 1, */                          \
            {0xcd06, "pll_pdec"}, /* PLL PDEC in direct control mode only, use_direct_pll_ctrl set \
                                     to 1, */                                                      \
            {0xcd70, "use_direct_pll_ctrl"}, /* Enabled PLL direct control mode, overrules the PLL \
                                                LUT with I2C register values, */                   \
            {0xcd80, "use_direct_clk_ctrl"}, /* Enabled CGU clock divider direct control mode , */                                                                                              \
            {0xcd90, "pll_limup_off"}, /* PLL up limiter control in PLL direct bandwidth control   \
                                          mode, pll_bandsel set to 1, */                           \
            {0xce0f, "tsig_freq_lsb"}, /* Internal sinus test generator frequency control   , */   \
            {0xcf02, "tsig_freq_msb"}, /* Select internal sinus test generator, frequency control  \
                                          msb bits, */                                             \
            ...
            {0xd201, "clkdiv_audio_sel"}, /* Audio clock divider selection in direct clock control \
                                             mode, */                                              \
            {0xd301, "int_ehs"},   /* Speed/load setting for INT IO cell, clk or data mode range   \
                                      (see SLIMMF IO datasheet), */                                \
            {0xd321, "datao_ehs"}, /* Speed/load setting for DATAO IO cell, clk or data mode range \
                                      (see SLIMMF IO datasheet), */                                \
            ...
            {0xd500, "source_in_testmode"}, /* TDM source in test mode (return only current and    \
                                               voltage sense), */                                  \
            {0xd510,                                                                               \
            ...
            {0xf987, "type_bits_fw"}, /* MTP control for firmware features - See Firmware I2C API  \
            ...
            {0xff07, "calibr_osc_delta_ndiv"}, /* Calibration data for OSC1M, signed number        \
                                                  representation, */                               \
            {0xffff, "Unknown bitfield enum"}  /* not found */                                     \
    };
#if 0
