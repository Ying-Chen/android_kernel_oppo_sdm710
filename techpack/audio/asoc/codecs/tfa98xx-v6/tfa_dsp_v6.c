...
int tfa_irq_set_pol(struct tfa_device* tfa, enum tfa9912_irq bit, int state) {
    ...
    if (bit == tfa9912_irq_all) {
        ...
        for (reg = TFA98XX_STATUS_POLARITY_REG1;
             reg <= TFA98XX_STATUS_POLARITY_REG1 + tfa9912_irq_max / 16; reg++) {
            tfa_reg_write(tfa, (unsigned char)reg, state ? 0xffff : 0); /* all bits */
        }
    } else if (bit < tfa9912_irq_max) {
        /* only this bit */
 ...
 */
enum Tfa98xx_DMEM tfa98xx_filter_mem_v6(struct tfa_device* tfa, int filter_index,
...
enum Tfa98xx_Error tfa98xx_supported_saam_v6(struct tfa_device* tfa, enum Tfa98xx_saam* saam) {
    ...
    if (tfa->support_saam == supportNotSet) {
        ...
        if (error != Tfa98xx_Error_Ok) return error;
        tfa->support_saam = (features & 0x8000) ? supportYes : supportNo; /* SAAM is bit15 */
    }
 ...
 */
enum Tfa98xx_Error tfa98xx_compare_features_v6(struct tfa_device* tfa, int features_from_MTP[3],
    ...
    if (!status) return Tfa98xx_Error_NoClock;  // Only test when we have a clock.
    ...
    /* Read SW features: */
    error = tfa_dsp_cmd_id_write_read_v6(tfa, MODULE_FRAMEWORK, FW_PAR_ID_GET_FEATURE_INFO,
                                         sizeof(bytes), bytes);
...
enum Tfa98xx_Error tfa98xx_init_v6(struct tfa_device* tfa) {
     ...
     */
    if (tfa->dev_ops.tfa_init) error = (tfa->dev_ops.tfa_init)(tfa);
...
int tfa98xx_powerswitch_is_enabled(struct tfa_device* tfa) {
    ...
    if (((tfa->rev & 0xff) == 0x13) || ((tfa->rev & 0xff) == 0x88)) {
        ...
        /* PLMA5539: Check actual value of powerswitch. TODO: regmap v1.40 should make this bit
         * public. */
...
enum Tfa98xx_Error tfa98xx_dsp_get_memory_v6(struct tfa_device* tfa, int memoryType, int offset,
...
enum Tfa98xx_Error tfa98xx_dsp_set_memory_v6(struct tfa_device* tfa, int memoryType, int offset,
...
void tfa2_manual_mtp_cpy(struct tfa_device* tfa, uint16_t reg_row_to_keep, uint16_t reg_row_to_set,
                         uint8_t row)  /// MCH_TO_TEST
{
    uint16_t value;
    int loop = 0;
    enum Tfa98xx_Error error;
    ...
    if (tfa->is_probus_device) {
        ...
        for (loop = 0; loop < 100 /*x10ms*/; loop++) {
            ...
            if (tfa_dev_get_mtpb(tfa) == 0) break;
        }
...
enum Tfa98xx_Error tfa98xx_set_mtp_v6(struct tfa_device* tfa, uint16_t value, uint16_t mask) {
    ...
    if (mtp_old == mtp_new) /* no change */ {
        if (tfa->verbose) pr_info("No change in MTP. Value not written! \n");
        return Tfa98xx_Error_Ok;
    }
...
enum Tfa98xx_Error tfa98xx_set_volume_level_v6(struct tfa_device* tfa, unsigned short vol) {
    if (tfa->in_use == 0) return Tfa98xx_Error_NotOpen;
...
static enum Tfa98xx_Error tfa98xx_set_mute_v6_tfa2(struct tfa_device* tfa, enum Tfa98xx_Mute mute) {
    enum Tfa98xx_Error error;

    if (tfa->dev_ops.set_mute == NULL) return Tfa98xx_Error_Not_Supported;
...
static enum Tfa98xx_Error tfa98xx_set_mute_v6_tfa1(struct tfa_device* tfa, enum Tfa98xx_Mute mute) {
    enum Tfa98xx_Error error;
...
enum Tfa98xx_Error tfa98xx_set_mute_v6(struct tfa_device* tfa, enum Tfa98xx_Mute mute) {
    if (tfa->in_use == 0) {
...
/****************** patching **********************************************************/
static enum Tfa98xx_Error tfa98xx_process_patch_file(struct tfa_device* tfa, int length,
    ...
    while (index < length) {
        ...
        if (error != Tfa98xx_Error_Ok) break;
        index += size;
    }
    return error;
}
 ...
 */
static enum Tfa98xx_Error tfa98xx_check_ic_rom_version(struct tfa_device* tfa,
    ...
    if ((checkrev != 0xFF) && (checkrev != lsb_revid)) return Tfa98xx_Error_Not_Supported;
    ...
    if (checkaddress != 0xFFFF) {
        ...
        if (error == Tfa98xx_Error_Ok) {
            if (value != checkvalue) {
                pr_err("patch file romid type check failed [0x%04x]: expected 0x%02x, actual "
                       "0x%02x\n",
...
enum Tfa98xx_Error tfa_dsp_patch_v6(struct tfa_device* tfa, int patchLength,
    ...
    if (patchLength < PATCH_HEADER_LENGTH) return Tfa98xx_Error_Bad_Parameter;
    ...
    error = tfa98xx_process_patch_file(tfa, patchLength - PATCH_HEADER_LENGTH,
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_wait_result_v6(struct tfa_device* tfa,
    ...
    do {
        ...
        if (cf_status < 0) error = -cf_status;
        tries++;
    }
    // i2c_cmd_ack
...
*/
void tfa98xx_convert_bytes2data_v6(int num_bytes, const unsigned char bytes[], int data[]) {
...
*/
void tfa98xx_convert_data2bytes_v6(int num_data, const int data[], unsigned char bytes[]) {
    int i; /* index for data */
    int k; /* index for bytes */
    int d;
...
// TODO properly handle bitfields: state should be restored! (now it will change eg dmesg field to
// xmem)
enum Tfa98xx_Error tfa_dsp_msg_v6_write_v6(struct tfa_device* tfa, int length, const char* buffer) {
    ...
    while ((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0)) {
        if (remaining_bytes < chunk_size) chunk_size = remaining_bytes;
        /* else chunk_size remains at initialize value above */
        error = tfa98xx_write_data_v6(tfa, FAM_TFA98XX_CF_MEM, chunk_size,
...
enum Tfa98xx_Error tfa_dsp_msg_v6_write_v6_id_v6(struct tfa_device* tfa, int length,
...
const char* tfa98xx_get_i2c_status_id_string_v6(int status) {
    ...
    switch (status) {
        ...
        case Tfa98xx_I2C_Req_Invalid_Seq:
            p_id_str =
                    "Invalid sequence of commands, in case the DSP expects some commands in a "
                    "specific order";
            break;
        ...
        case Tfa98xx_I2C_Req_Buffer_Overflow:
            p_id_str =
                    "I2C buffer has overflowed: host has sent too many parameters, memory "
...
enum Tfa98xx_Error tfa_dsp_msg_v6_read_v6(struct tfa_device* tfa, int length,
    ...
    if (length > TFA2_MAX_PARAM_SIZE) return Tfa98xx_Error_Bad_Parameter;
    ...
    while (num_bytes > 0) {
        ...
        if (num_bytes < burst_size) burst_size = num_bytes;
...
enum Tfa98xx_Error dsp_msg(struct tfa_device* tfa, int length24, const char* buf24) {
    ...
    if (error != Tfa98xx_Error_Ok)
        error = (enum Tfa98xx_Error)(
                error + Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */

    /* DSP verbose has argument 0x04 */
    if ((tfa->verbose & 0x04) != 0) {
        pr_debug("DSP w [%d]: ", length);
        for (i = 0; i < length; i++) pr_debug("0x%02x ", (uint8_t)buf[i]);
        pr_debug("\n");
    }
...
enum Tfa98xx_Error dsp_msg_read(struct tfa_device* tfa, int length24, unsigned char* bytes24) {
    ...
    if ((tfa->verbose & 0x04) != 0) {
        ...
        for (i = 0; i < length; i++) pr_debug("0x%02x ", (uint8_t)bytes[i]);
        pr_debug("\n");
    }
...
enum Tfa98xx_Error mem_read(struct tfa_device* tfa, unsigned int start_offset, int num_words,
                            int* pValues) {
    enum Tfa98xx_Error error;
...
enum Tfa98xx_Error mem_write(struct tfa_device* tfa, unsigned short address, int value,
                             int memtype) {
    enum Tfa98xx_Error error;
...
enum Tfa98xx_Error tfa_dsp_msg_v6(struct tfa_device* tfa, int length, const char* buf) {
    ...
    for (tries = TFA98XX_WAITRESULT_NTRIES; tries > 0; tries--) {
        ...
        if (error == Tfa98xx_Error_Ok && rpc_status == Tfa98xx_I2C_Req_Done) break;
        /* If the rpc status is a specific error we want to know it.
         * If it is busy or not running it should retry
         */
        if (rpc_status != Tfa98xx_I2C_Req_Busy && rpc_status != Tfa98xx_DSP_Not_Running) break;
    }
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_id_v6(struct tfa_device* tfa, int length, const char* buf,
                                        uint8_t cmdid[3]) {
    enum Tfa98xx_Error error;
    int tries, rpc_status = Tfa98xx_I2C_Req_Done;
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_check_rpc_status_v6(struct tfa_device* tfa,
                                                            int* pRpcStatus) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    ...
    if (tfa->in_use == 0) return Tfa98xx_Error_NotOpen;
    if (pRpcStatus == NULL) return Tfa98xx_Error_Bad_Parameter;
...
/***************************** xmem only **********************************/
enum Tfa98xx_Error tfa98xx_dsp_read_mem_v6(struct tfa_device* tfa, unsigned int start_offset,
    ...
    if (bytes == NULL) return Tfa98xx_Error_Fail;
    ...
    if (error != Tfa98xx_Error_Ok) goto tfa98xx_dsp_read_mem_v6_exit;
...
enum Tfa98xx_Error tfa98xx_dsp_write_mem_word_v6(struct tfa_device* tfa, unsigned short address,
...
enum Tfa98xx_Error Tfa98xx_DspBiquad_Disable_v6(struct tfa_device* tfa, int biquad_index) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    int coeff_buffer[BIQUAD_COEFF_SIZE];
    unsigned char bytes[3 + BIQUAD_COEFF_SIZE * 3];
    int nr = 0;

    if (biquad_index > TFA98XX_BIQUAD_NUM) return Tfa98xx_Error_Bad_Parameter;
    if (biquad_index < 1) return Tfa98xx_Error_Bad_Parameter;
...
enum Tfa98xx_Error tfa_dsp_cmd_id_write_v6(struct tfa_device* tfa, unsigned char module_id,
    ...
    if (buffer == NULL) return Tfa98xx_Error_Fail;
...
enum Tfa98xx_Error tfa_dsp_cmd_id_write_read_v6(struct tfa_device* tfa, unsigned char module_id,
                                                unsigned char param_id, int num_bytes,
                                                unsigned char data[]) {
    enum Tfa98xx_Error error;
    ...
    if ((tfa->is_probus_device) && (tfa->cnt->ndev == 1) &&
        (param_id == SB_PARAM_GET_RE25C || param_id == SB_PARAM_GET_LSMODEL ||
...
enum Tfa98xx_Error tfa_dsp_cmd_id_coefs_v6(struct tfa_device* tfa, unsigned char module_id,
                                           unsigned char param_id, int num_bytes,
                                           unsigned char data[]) {
    enum Tfa98xx_Error error;
...
enum Tfa98xx_Error tfa_dsp_cmd_id_MBDrc_dynamics_v6(struct tfa_device* tfa, unsigned char module_id,
...
enum Tfa98xx_Error tfa98xx_dsp_write_preset_v6_v6(struct tfa_device* tfa, int length,
                                                  const unsigned char* p_preset_bytes) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    if (p_preset_bytes != NULL) {
        /* by design: keep the data opaque and no
         * interpreting/calculation */
        error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_PRESET, length,
                                        p_preset_bytes);
    } else {
 ...
 */
enum Tfa98xx_Error tfa98xx_dsp_get_hw_feature_bits_v6(struct tfa_device* tfa, int* features) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    uint32_t value;
    uint16_t mtpbf;
...
enum Tfa98xx_Error tfa98xx_dsp_get_sw_feature_bits_v6(struct tfa_device* tfa, int features[2]) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa98xx_dsp_get_state_info_v6(struct tfa_device* tfa, unsigned char bytes[],
    ...
    if (err == Tfa98xx_Error_Ok) {
        if (bSupportFramework) {
            err = tfa_dsp_cmd_id_write_read_v6(tfa, MODULE_FRAMEWORK, FW_PARAM_GET_STATE,
                                               3 * stateSize, bytes);
        } else {
            /* old ROM code, ask SpeakerBoost and only do first portion */
            stateSize = 8;
            err = tfa_dsp_cmd_id_write_read_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_STATE,
                                               3 * stateSize, bytes);
        }
    }
...
enum Tfa98xx_Error tfa98xx_dsp_write_speaker_parameters_v6(struct tfa_device* tfa, int length,
    ...
    if (p_speaker_bytes != NULL) {
        ...
        error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_LSMODEL, length,
                                        p_speaker_bytes);
    } else {
        error = Tfa98xx_Error_Bad_Parameter;
    }
    ...
    if (bSupportDrc) {
        ...
        error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_AGCINS, 3, bytes);
    }

    return error;
}

enum Tfa98xx_Error tfa98xx_dsp_write_config_v6(struct tfa_device* tfa, int length,
                                               const unsigned char* p_config_bytes) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    int bSupportDrc;

    error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_CONFIG, length,
                                    p_config_bytes);
    if (error != Tfa98xx_Error_Ok) return error;
...
enum Tfa98xx_Error tfa98xx_dsp_write_drc_v6(struct tfa_device* tfa, int length,
    ...
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    if (p_drc_bytes != NULL) {
        error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_DRC, length,
                                        p_drc_bytes);
...
enum Tfa98xx_Error tfa_cf_powerup_v6(struct tfa_device* tfa) {
    ...
    if (tfa->verbose) pr_info("Waiting for DSP system stable...\n");
...
static enum Tfa98xx_Error tfa98xx_aec_output(struct tfa_device* tfa, int enable) {
    ...
    if ((tfa->daimap & Tfa98xx_DAI_TDM) == Tfa98xx_DAI_TDM) return err;
...
enum Tfa98xx_Error tfaGetFwApiVersion(struct tfa_device* tfa, unsigned char* pFirmwareVersion) {
    ...
    if (tfa == NULL) return Tfa98xx_Error_Bad_Parameter;
    if (!tfa->is_probus_device) {
...
enum Tfa98xx_Error tfaRunSpeakerBoost_v6(struct tfa_device* tfa, int force, int profile) {
    ...
    if (value) {
        ...
        if ((tfa->tfa_family == 1) && (tfa->daimap == Tfa98xx_DAI_TDM)) tfa->sync_iv_delay = 1;
...
enum Tfa98xx_Error tfaRunSpeakerStartup_v6(struct tfa_device* tfa, int force, int profile) {
    ...
    if (!force) {  // in case of force CF already runnning
        ...
        if (tfa_cf_enabled(tfa) == 0) return err;
...
enum Tfa98xx_Error tfaRunSpeakerCalibration_v6(struct tfa_device* tfa) {
    ...
    if ((tfa->tfa_family == 2) && TFA_GET_BF(tfa, NOCLK)) return Tfa98xx_Error_NoClock;
...
enum Tfa98xx_Error tfaRunStartup_v6(struct tfa_device* tfa, int profile) {
    ...
    if (dev == NULL) return Tfa98xx_Error_Fail;
...
enum Tfa98xx_Error tfaRunMute_v6(struct tfa_device* tfa) {
    ...
    if (tfa->tfa_family == 1) {
        ...
        if (err == Tfa98xx_Error_Ok) {
            ...
            if (tfa->verbose) pr_debug("-------------------- muted --------------------\n");

            /*The amplifier is always switching*/
            if (tries == AMPOFFWAIT_TRIES) return Tfa98xx_Error_Other;
        }
    }
...
enum Tfa98xx_Error tfaRunUnmute_v6(struct tfa_device* tfa) {
    ...
    if (tfa->verbose) pr_debug("-------------------unmuted ------------------\n");
...
enum tfa_error tfa_dev_start(struct tfa_device* tfa, int next_profile, int vstep) {
    ...
    if (active_profile == 0xff) active_profile = -1;
    ...
    if (tfa->tfa_family == 1) { /* TODO move this to ini file */
        ...
        if (err != Tfa98xx_Error_Ok) goto error_exit;
    }
...
enum tfa_error tfa_dev_stop(struct tfa_device* tfa) {
    ...
    if (err != Tfa98xx_Error_Ok) return (enum tfa_error)err;
...
int tfa_reset_v6(struct tfa_device* tfa) {
    ...
    /* Check device state. Print warning if reset is done from other state than powerdown (when
     * verbose) */
 ...
 */
enum Tfa98xx_Error tfa98xx_write_data_v6(struct tfa_device* tfa, unsigned char subaddress,
    ...
    if (num_bytes > TFA2_MAX_PARAM_SIZE) return Tfa98xx_Error_Bad_Parameter;

    write_data = (unsigned char*)kmem_cache_alloc(tfa->cachep, GFP_KERNEL);
    if (write_data == NULL) return Tfa98xx_Error_Fail;
...
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance_v6(struct tfa_device* tfa) {
    ...
    if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC) && !aging_flag) {
        ...
        error = tfa_dsp_cmd_id_write_read_v6(tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_RE25C, nr_bytes,
                                             bytes);
        if (error == Tfa98xx_Error_Ok) {
    ...
    if (new_cost >= old_cost) {
        ...
        for (eq = 0; eq < NR_EQ; eq++) {
            ...
            if (eq_biquad_mask[eq] == 0xffffffff) {
                ...
                if (msg == NULL) return Tfa98xx_Error_Fail;
...
enum tfa_error tfa_dev_set_state(struct tfa_device* tfa, enum tfa_state state) {
    ...
    switch (state & 0x0f) {
        ...
        case TFA_STATE_INIT_CF: /* coolflux HW access possible (~initcf) */
            ...
            if (!tfa->is_probus_device) TFA_SET_BF(tfa, SBSL, 0);

            /* We want to leave Wait4SrcSettings state for max2 */
            if (tfa->tfa_family == 2) TFA_SET_BF(tfa, MANSCONF, 1);
            ...
            do {
                ...
                if (err != tfa_error_ok) return err;
                if (ready) break;
            } while (loop--);

            if (!tfa->is_probus_device) {
                err = (enum tfa_error)tfa98xx_set_mtp_v6(tfa, 1,
        ...
        case TFA_STATE_OPERATING:     /* Amp and Algo running */
             ...
             * However in case of calibration wait for DSP! (This should be case only during
             * calibration).
             */
            ...
            if (tfa->sync_iv_delay) {
                if (tfa->verbose) pr_debug("syncing I/V delay for %x\n", (tfa->rev & 0xff));
        ...
        case TFA_STATE_RESET: /* I2C reset and ACS set */
            ...
            break;
        default:
            if (state & 0x0f) return tfa_error_bad_param;
    }

    /* state modifiers */

    if (state & TFA_STATE_MUTE) tfa98xx_set_mute_v6(tfa, Tfa98xx_Mute_Amplifier);

    if (state & TFA_STATE_UNMUTE) tfa98xx_set_mute_v6(tfa, Tfa98xx_Mute_Off);
...
enum tfa_error tfa_dev_mtp_set(struct tfa_device* tfa, enum tfa_mtp item, int value) {
    ...
    switch (item) {
        case TFA_MTP_OTC:
            err = (enum tfa_error)tfa98xx_set_mtp_v6(tfa, (uint16_t)value,
                                                     TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK);
            break;
        case TFA_MTP_EX:
            err = (enum tfa_error)tfa98xx_set_mtp_v6(tfa, (uint16_t)value,
        ...
        case TFA_MTP_RE25_PRIM:
            if (tfa->tfa_family == 2) {
                ...
                if ((tfa->rev & 0xFF) == 0x88)
                    ...
                    if (tfa->is_probus_device == 1) tfa2_manual_mtp_cpy(tfa, 0xF4, value, 2);
...
enum Tfa98xx_Error tfa_status(struct tfa_device* tfa) {
    ...
    if (TFA_GET_BF_VALUE(tfa, ACS, val) || TFA_GET_BF_VALUE(tfa, WDS, val)) {
        if (TFA_GET_BF_VALUE(tfa, ACS, val)) pr_err("ERROR: ACS\n");
        if (TFA_GET_BF_VALUE(tfa, WDS, val)) pr_err("ERROR: WDS\n");

        return Tfa98xx_Error_DSP_not_running;
    }

    if (TFA_GET_BF_VALUE(tfa, SPKS, val)) pr_err("ERROR: SPKS\n");
    if (!TFA_GET_BF_VALUE(tfa, SWS, val)) pr_err("ERROR: SWS\n");

    /* Check secondary errors */
    if (!TFA_GET_BF_VALUE(tfa, CLKS, val) || !TFA_GET_BF_VALUE(tfa, UVDS, val) ||
        !TFA_GET_BF_VALUE(tfa, OVDS, val) || !TFA_GET_BF_VALUE(tfa, OTDS, val) ||
        !TFA_GET_BF_VALUE(tfa, PLLS, val) ||
        (!(tfa->daimap & Tfa98xx_DAI_TDM) && !TFA_GET_BF_VALUE(tfa, VDDS, val)))
    ...
    if ((tfa->daimap & Tfa98xx_DAI_TDM) && (tfa->tfa_family == 2)) {
        ...
        if (value < 0) return -value;
        val = (uint16_t)value;
        if (TFA_GET_BF_VALUE(tfa, TDMERR, val) || TFA_GET_BF_VALUE(tfa, TDMLUTER, val))
...
void tfa_adapt_noisemode(struct tfa_device* tfa) {
    ...
    if (tfa->verbose) pr_debug("Adapting low noise mode\n");

    for (i = 0; i < NR_OF_BATS; i++) {
        int bat = TFA_GET_BF(tfa, BATS);
        if (tfa->verbose) pr_debug("bats[%d]=%d\n", i, bat);
        total_bats += bat;
        msleep_interruptible(5);
    }
    ...
    if (avbatt <= MIN_BATT_LEVEL &&
        !tfa_get_bf_v6(tfa, TFA9873_BF_LNMODE))  // 640 corresponds to 3.4 volt, MCH_TO_TEST
    {
