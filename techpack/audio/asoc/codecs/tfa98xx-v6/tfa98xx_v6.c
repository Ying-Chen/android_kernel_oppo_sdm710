...
static bool is_tfa98xx_series(int rev) {
    ...
    if (((rev & 0xff) == 0x80) || ((rev & 0xff) == 0x81) || ((rev & 0xff) == 0x92) ||
        ((rev & 0xff) == 0x91) || ((rev & 0xff) == 0x94) || ((rev & 0xff) == 0x73) ||
        ((rev & 0xff) == 0x74)) {
        ret = true;
    }
...
static int ftm_spk_rev_get(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    int rev = 0;
    int ret;
...
static int ftm_spk_rev_put(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    return 0;
}

static const struct snd_kcontrol_new ftm_spk_rev_controls[] = {
        SOC_ENUM_EXT("SPK_Pa Revision", ftm_spk_rev_enum, ftm_spk_rev_get, ftm_spk_rev_put),
};
...
static const struct snd_kcontrol_new tfa98xx_snd_controls[] = {
        SOC_SINGLE_EXT("TFA98XX Volume", SND_SOC_NOPM, 0, 0xff, 0, tfa98xx_volume_get,
                       tfa98xx_volume_set),
        SOC_SINGLE_EXT("TFA98XX ANA Volume", SND_SOC_NOPM, 0, 0xff, 0, tfa98xx_ana_volume_get,
                       tfa98xx_ana_volume_set),
};

static inline char* tfa_cont_profile_name(struct tfa98xx* tfa98xx, int prof_idx) {
    if (tfa98xx->tfa->cnt == NULL) return NULL;
...
static ssize_t kernel_debug_read(struct file* file, char __user* buf, size_t count, loff_t* pos) {
...
static ssize_t kernel_debug_write(struct file* f, const char __user* buf, size_t count,
                                  loff_t* offset) {
    pr_info("%s \n", __FUNCTION__);
    return 0;
}
...
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx* tfa98xx_cal, int* speakerImpedance) {
    ...
    if (tfa98xx->tfa->dev_idx == 0) {
        ...
        memset(bytes, 0, 6);
        err = tfa_dsp_cmd_id_write_v6(tfa98xx->tfa, MODULE_SPEAKERBOOST, SB_PARAM_SET_RE25C,
                                      sizeof(bytes), bytes);
        ...
        if (tfa98xx_device_count == 2) {
            ...
            list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
                if (tfa98xx->tfa->channel != 0xff) {
                    /*if channel initialized, update impendance to the left/right device through
                     * tfa->channel*/
                    /*tfa98xx point to different device, but both of them use mohm[0] to save
                     * impendance value*/
                    ...
                    /*if channel not initialized, set as defalut, cnt-id 0 is left, cnt-id 1 is
                     * right*/
                    if (tfa98xx->tfa->dev_idx == 0)
                        tfa98xx->tfa->mohm[0] = impLR[0];
                    else
        ...
        *speakerImpedance = tfa_get_calibration_info_v6(tfa98xx->tfa, 0);
        ...
        pr_info("%s: devid 0 0x%x Calibration value: %d.%3d ohm.\n", __func__,
                tfa98xx_cal->i2c->addr, *speakerImpedance / 1000, *speakerImpedance % 1000);
        ...
        list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
            ...
            if (!aging_flag) {
                err = (enum Tfa98xx_Error)tfa_dev_mtp_set(tfa98xx->tfa, TFA_MTP_RE25,
                                                          (uint16_t)((int)(imp)));
                if (err) {
...
static int tfa98xx_input_open(struct input_dev* dev) {
    ...
    if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
        dev_info(&tfa98xx->i2c->dev, "DSP not loaded, cannot start tap-detection\n");
        return -EIO;
    }
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx* tfa98xx, bool unregister) {
    ...
    for (i = 0; i < tfa_cnt_get_dev_nprof(tfa98xx->tfa); i++) {
        if (strstr(tfa_cont_profile_name(tfa98xx, i), ".tap")) {
            ...
            dev_info(tfa98xx->component->dev, "found a tap-detection profile (%d - %s)\n", i,
     ...
     */
    if (!(tfa98xx->flags & TFA98XX_FLAG_TAPDET_AVAILABLE) || !tap_profile || unregister) {
...
static ssize_t tfa98xx_dbgfs_mtpex_get(struct file* file, char __user* user_buf, size_t count,
                                       loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
...
static ssize_t tfa98xx_dbgfs_start_set(struct file* file, const char __user* user_buf, size_t count,
                                       loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
    ...
    if (count > sizeof(ref) + 1 || count < (sizeof(ref) - 1)) return -EINVAL;
    ...
    if (strncmp(buf, ref, sizeof(ref) - 1)) return -EINVAL;

    mutex_lock(&tfa98xx->dsp_lock);
    ret = tfa_calibrate_v6(tfa98xx->tfa);
    if (ret == tfa_error_ok) ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
    if (ret == tfa_error_ok) tfa_dev_set_state(tfa98xx->tfa, TFA_STATE_UNMUTE);
    mutex_unlock(&tfa98xx->dsp_lock);
...
static ssize_t tfa98xx_dbgfs_r_read(struct file* file, char __user* user_buf, size_t count,
                                    loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
...
static ssize_t tfa98xx_dbgfs_range_read(struct file* file, char __user* user_buf, size_t count,
                                        loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
...
static ssize_t tfa98xx_dbgfs_r_aging_read(struct file* file, char __user* user_buf, size_t count,
                                          loff_t* ppos) {
    int ret = 0;
...
static ssize_t tfa98xx_dbgfs_r_impedance_read(struct file* file, char __user* user_buf,
...
static ssize_t tfa98xx_dbgfs_version_read(struct file* file, char __user* user_buf, size_t count,
                                          loff_t* ppos) {
...
static ssize_t tfa98xx_dbgfs_dsp_state_get(struct file* file, char __user* user_buf, size_t count,
                                           loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
...
static ssize_t tfa98xx_dbgfs_dsp_state_set(struct file* file, const char __user* user_buf,
                                           size_t count, loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
    ...
    if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
        ...
        queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->monitor_work, HZ);
...
static ssize_t tfa98xx_dbgfs_fw_state_get(struct file* file, char __user* user_buf, size_t count,
                                          loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
...
static ssize_t tfa98xx_dbgfs_rpc_read(struct file* file, char __user* user_buf, size_t count,
                                      loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
...
static ssize_t tfa98xx_dbgfs_rpc_send(struct file* file, const char __user* user_buf, size_t count,
                                      loff_t* ppos) {
#ifdef CONFIG_DEBUG_FS
    struct i2c_client* i2c = file->private_data;
#else  /*CONFIG_DEBUG_FS*/
    ...
    if (tfa98xx->tfa->is_probus_device) {
        ...
        if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
            error = tfaContWriteFile_v6(tfa98xx->tfa, msg_file, 0,
...
DEFINE_SIMPLE_ATTRIBUTE(tfa98xx_dbgfs_calib_otc_fops, tfa98xx_dbgfs_otc_get, tfa98xx_dbgfs_otc_set,
                        "%llu\n");
...
static void tfa98xx_debug_init(struct tfa98xx* tfa98xx, struct i2c_client* i2c) {
    ...
    debugfs_create_file("calibrate", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, i2c,
                        &tfa98xx_dbgfs_calib_start_fops);
    debugfs_create_file("R", S_IRUGO, tfa98xx->dbg_dir, i2c, &tfa98xx_dbgfs_r_fops);
    debugfs_create_file("range", S_IRUGO, tfa98xx->dbg_dir, i2c, &tfa98xx_dbgfs_range_fops);
    debugfs_create_file("r_aging", S_IRUGO, tfa98xx->dbg_dir, i2c, &tfa98xx_dbgfs_r_aging_fops);
    debugfs_create_file("r_impedance", S_IRUGO, tfa98xx->dbg_dir, i2c,
                        &tfa98xx_dbgfs_r_impedance_fops);
    debugfs_create_file("version", S_IRUGO, tfa98xx->dbg_dir, i2c, &tfa98xx_dbgfs_version_fops);
    debugfs_create_file("dsp-state", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, i2c,
                        &tfa98xx_dbgfs_dsp_state_fops);
    debugfs_create_file("fw-state", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, i2c,
                        &tfa98xx_dbgfs_fw_state_fops);
    debugfs_create_file("rpc", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, i2c, &tfa98xx_dbgfs_rpc_fops);

    if (tfa98xx->flags & TFA98XX_FLAG_SAAM_AVAILABLE) {
        dev_dbg(tfa98xx->dev, "Adding pga_gain debug interface\n");
        debugfs_create_file("pga_gain", S_IRUGO, tfa98xx->dbg_dir, tfa98xx->i2c,
                            &tfa98xx_dbgfs_pga_gain_fops);
    }
#else  /*CONFIG_DEBUG_FS*/
    tfa98xx->dbg_dir = proc_mkdir(name, NULL);
    proc_create_data("OTC", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_calib_otc_fops,
                     i2c);
    proc_create_data("MTPEX", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_calib_mtpex_fops,
                     i2c);
    proc_create_data("TEMP", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_calib_temp_fops,
                     i2c);
    proc_create_data("calibrate", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir,
                     &tfa98xx_dbgfs_calib_start_fops, i2c);
    proc_create_data("R", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_r_fops, i2c);
    proc_create_data("range", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_range_fops, i2c);
    proc_create_data("r_aging", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_r_aging_fops, i2c);
    proc_create_data("r_impedance", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_r_impedance_fops,
                     i2c);
    proc_create_data("version", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_version_fops, i2c);
    proc_create_data("dsp-state", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir,
                     &tfa98xx_dbgfs_dsp_state_fops, i2c);
    proc_create_data("fw-state", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_fw_state_fops,
                     i2c);
    proc_create_data("rpc", S_IRUGO | S_IWUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_rpc_fops, i2c);

    if (tfa98xx->flags & TFA98XX_FLAG_SAAM_AVAILABLE) {
        dev_dbg(tfa98xx->dev, "Adding pga_gain debug interface\n");
        proc_create_data("pga_gain", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_pga_gain_fops,
                         tfa98xx->i2c);
    }
#endif /*CONFIG_DEBUG_FS*/
}
...
#ifdef CONFIG_DEBUG_FS
    if (tfa98xx->dbg_dir) debugfs_remove_recursive(tfa98xx->dbg_dir);
#else
    if (tfa98xx->dbg_dir) proc_remove(tfa98xx->dbg_dir);
...
static int is_profile_in_list(char* profile, int len) {
    ...
    list_for_each_entry(bprof, &profile_list, list) {
        if ((len == bprof->len) && (0 == strncmp(bprof->basename, profile, len))) return 1;
    }
...
static int is_calibration_profile(char* profile) {
    if (strstr(profile, ".cal") != NULL) return 1;
    return 0;
}
...
static int tfa98xx_get_vstep(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct snd_soc_component* component = snd_soc_kcontrol_component(kcontrol);
    struct tfa98xx* tfa98xx = snd_soc_component_get_drvdata(component);
    ...
    list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        ...
        ucontrol->value.integer.value[tfa98xx->tfa->dev_idx] =
                tfacont_get_max_vstep_v6(tfa98xx->tfa, profile) - vstep - 1;
    }
...
static int tfa98xx_set_vstep(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct snd_soc_component* component = snd_soc_kcontrol_component(kcontrol);
    struct tfa98xx* tfa98xx = snd_soc_component_get_drvdata(component);
    int mixer_profile = kcontrol->private_value;
    int profile;
    ...
    list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        ...
        if (vstep == vsteps - value - 1) continue;

        new_vstep = vsteps - value - 1;

        if (new_vstep < 0) new_vstep = 0;
...
static int tfa98xx_info_vstep(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_info* uinfo) {
    struct snd_soc_component* component = snd_soc_kcontrol_component(kcontrol);
    struct tfa98xx* tfa98xx = snd_soc_component_get_drvdata(component);
...
static int tfa98xx_get_profile(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    mutex_lock(&tfa98xx_mutex);
...
static int tfa98xx_set_profile(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct snd_soc_component* component = snd_soc_kcontrol_component(kcontrol);
    struct tfa98xx* tfa98xx = snd_soc_component_get_drvdata(component);
    int change = 0;
    int new_profile;
...
static int tfa98xx_info_profile(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_info* uinfo) {
    ...
    if (uinfo->value.enumerated.item >= count) uinfo->value.enumerated.item = count - 1;
...
static int tfa98xx_info_stop_ctl(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_info* uinfo) {
...
static int tfa98xx_set_stop_ctl(struct snd_kcontrol* kcontrol,
    ...
    list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        ...
        if ((ucontrol->value.integer.value[i] != 0) && ready) {
            ...
            if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) continue;
...
static int tfa98xx_info_cal_ctl(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_info* uinfo) {
    uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
    mutex_lock(&tfa98xx_mutex);
    uinfo->count = tfa98xx_device_count;
    mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_set_cal_ctl(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct tfa98xx* tfa98xx;
...
static int tfa98xx_get_cal_ctl(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_value* ucontrol) {
    struct tfa98xx* tfa98xx;
...
static int tfa98xx_info_stereo_ctl(struct snd_kcontrol* kcontrol, struct snd_ctl_elem_info* uinfo) {
    uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
    uinfo->count = 1;
    uinfo->value.integer.min = 0;
...
static int tfa98xx_create_controls(struct tfa98xx* tfa98xx) {
    ...
    if (tfa98xx->flags & TFA98XX_FLAG_CALIBRATION_CTL) nr_controls += 1; /* calibration */
    ...
    if (!tfa98xx_controls) return -ENOMEM;
    ...
    for (prof = 0; prof < nprof; prof++) {
        ...
        if ((is_profile_in_list(bprofile->basename, bprofile->len) == 0) &&
            ...
            if (tfacont_get_max_vstep_v6(tfa98xx->tfa, prof)) {
                ...
                scnprintf(name, MAX_CONTROL_NAME, "%s %s Playback Volume", tfa98xx->fw.name,
                          bprofile->basename);
    ...
    return snd_soc_add_component_controls(tfa98xx->component, tfa98xx_controls, mix_index);
}
...
static int tfa98xx_append_i2c_address(struct device* dev, struct i2c_client* i2c,
    ...
    if (dai_drv && num_dai > 0)
        for (i = 0; i < num_dai; i++) {
            ...
            snprintf(buf, 50, "%s-%x-%x", dai_drv[i].playback.stream_name, i2cbus, addr);
            dai_drv[i].playback.stream_name = tfa98xx_devm_kstrdup(dev, buf);

            snprintf(buf, 50, "%s-%x-%x", dai_drv[i].capture.stream_name, i2cbus, addr);
    ...
    if (widgets && num_widgets > 0)
        for (i = 0; i < num_widgets; i++) {
            if (!widgets[i].sname) continue;
            if ((widgets[i].id == snd_soc_dapm_aif_in) || (widgets[i].id == snd_soc_dapm_aif_out)) {
...
static void tfa98xx_add_widgets(struct tfa98xx* tfa98xx) {
    ...
    widgets = devm_kzalloc(
            &tfa98xx->i2c->dev,
            sizeof(struct snd_soc_dapm_widget) * ARRAY_SIZE(tfa98xx_dapm_widgets_common),
            GFP_KERNEL);
    if (!widgets) return;
    memcpy(widgets, tfa98xx_dapm_widgets_common,
           sizeof(struct snd_soc_dapm_widget) * ARRAY_SIZE(tfa98xx_dapm_widgets_common));

    tfa98xx_append_i2c_address(&tfa98xx->i2c->dev, tfa98xx->i2c, widgets, num_dapm_widgets, NULL,
                               0);

    snd_soc_dapm_new_controls(dapm, widgets, ARRAY_SIZE(tfa98xx_dapm_widgets_common));
...
enum Tfa98xx_Error tfa98xx_write_register16_v6(struct tfa_device* tfa, unsigned char subaddress,
...
enum Tfa98xx_Error tfa98xx_read_register16_v6(struct tfa_device* tfa, unsigned char subaddress,
                                              unsigned short* val) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    struct tfa98xx* tfa98xx;
    unsigned int value;
 ...
 */
enum Tfa98xx_Error tfa98xx_writeread_dsp(struct tfa_device* tfa, int command_length,
                                         void* command_buffer, int result_length,
                                         void* result_buffer) {
    return Tfa98xx_Error_Not_Supported;
}

enum Tfa98xx_Error tfa98xx_read_data_v6(struct tfa_device* tfa, unsigned char reg, int len,
                                        unsigned char value[]) {
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    struct tfa98xx* tfa98xx;
    ...
    if (tfa98xx->i2c) {
        ...
        do {
            err = i2c_transfer(tfa98xx_client->adapter, msgs, ARRAY_SIZE(msgs));
            if (err != ARRAY_SIZE(msgs)) msleep_interruptible(I2C_RETRY_DELAY);
        ...
        if (tfa98xx_ftrace_regs) tfa98xx_trace_printk("\t\tRD-DAT reg=0x%02x, len=%d\n", reg, len);
    } else {
        pr_err("No device available\n");
        error = Tfa98xx_Error_Fail;
    }
...
retry:
    ...
    if (ret == len) {
        if (tfa98xx_kmsg_regs) dev_dbg(&tfa98xx->i2c->dev, "  WR-RAW len=%d\n", len);
        if (tfa98xx_ftrace_regs) tfa98xx_trace_printk("\t\tWR-RAW len=%d\n", len);
        return Tfa98xx_Error_Ok;
    }
...
static void tfa98xx_tapdet_check_update(struct tfa98xx* tfa98xx) {
    ...
    if ((tfa98xx->flags & TFA98XX_FLAG_TAPDET_AVAILABLE) == 0) return;

    if (tfa98xx->tapdet_open && (tfa98xx->tapdet_profiles & (1 << tfa98xx->profile))) enable = true;
    ...
    if (!gpio_is_valid(tfa98xx->irq_gpio)) {
        ...
        dev_dbg(tfa98xx->component->dev, "Polling for tap-detection: %s (%d; 0x%x, %d)\n",
                enable ? "enabled" : "disabled", tfa98xx->tapdet_open, tfa98xx->tapdet_profiles,
                tfa98xx->profile);

    } else {
        dev_dbg(tfa98xx->component->dev, "Interrupt for tap-detection: %s (%d; 0x%x, %d)\n",
                enable ? "enabled" : "disabled", tfa98xx->tapdet_open, tfa98xx->tapdet_profiles,
                tfa98xx->profile);
        /*  enabled interrupt */
...
static void tfa98xx_interrupt_enable(struct tfa98xx* tfa98xx, bool enable) {
    if (tfa98xx->flags & TFA98XX_FLAG_SKIP_INTERRUPTS) return;

    if (tfa98xx->tfa->tfa_family == 2) tfa98xx_interrupt_enable_tfa2(tfa98xx, enable);
}
...
static void tfa98xx_container_loaded(const struct firmware* cont, void* context) {
    ...
    */
    ...
    if (strcmp(dflt_prof_name, "")) {
        ...
        for (i = 0; i < nprof; i++) {
            if (strcmp(tfa_cont_profile_name(tfa98xx, i), dflt_prof_name) == 0) {
                tfa98xx->profile = i;
        ...
        if (i >= nprof)
            dev_info(tfa98xx->dev, "Default profile override failed (%s profile not found)\n",
                     dflt_prof_name);
    }
    ...
    if (no_start != 0) return;

    /* Only controls for master device */
    if (tfa98xx->tfa->dev_idx == 0) tfa98xx_create_controls(tfa98xx);
    ...
    if (tfa98xx->tfa->tfa_family == 2) {
        mutex_lock(&tfa98xx->dsp_lock);
        ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
        if (ret == Tfa98xx_Error_Not_Supported) tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL;
        mutex_unlock(&tfa98xx->dsp_lock);
    }
...
static int tfa98xx_load_container(struct tfa98xx* tfa98xx) {
    ...
    return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, fw_name, tfa98xx->dev,
                                   GFP_KERNEL, tfa98xx, tfa98xx_container_loaded);
}
...
static void tfa98xx_tapdet_work(struct work_struct* work) {
    ...
    if (tfa_irq_get(tfa98xx->tfa, tfa9912_irq_sttapdet)) tfa98xx_tapdet(tfa98xx);
...
static void tfa98xx_dsp_init(struct tfa98xx* tfa98xx) {
    ...
    if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
        ...
        if (ret == Tfa98xx_Error_Not_Supported) {
             ...
             */
            dev_err(&tfa98xx->i2c->dev, "tfa_dev_start failed! (err %d) - %d\n", ret,
                    tfa98xx->init_count);
            reschedule = true;
        } else {
            sync = true;
            ...
            dev_info(&tfa98xx->i2c->dev, "tfa_dev_start success (%d)\n", tfa98xx->init_count);
    ...
    if (reschedule) {
        ...
        queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->init_work, msecs_to_jiffies(5));
        tfa98xx->init_count++;
    }
    if (failed) {
    ...
    if (sync) {
        ...
        if (tfa98xx_sync_count < tfa98xx_device_count) tfa98xx_sync_count++;
        ...
        if (do_sync) {
            ...
            list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
                ...
                if (tfa98xx->tfa->tfa_family == 1)
                    queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->monitor_work, 1 * HZ);
                mutex_unlock(&tfa98xx->dsp_lock);
            }
        }
...
static void tfa98xx_interrupt(struct work_struct* work) {
    ...
    if (tfa98xx->flags & TFA98XX_FLAG_REMOVE_PLOP_NOISE) {
        ...
        if (start_triggered) tfa98xx_interrupt_enable(tfa98xx, true);
        mutex_unlock(&tfa98xx->dsp_lock);
...
static int tfa98xx_startup(struct snd_pcm_substream* substream, struct snd_soc_dai* dai) {
    ...
    if (pcm_no_constraint != 0) return 0;
    ...
    err = snd_pcm_hw_constraint_mask64(substream->runtime, SNDRV_PCM_HW_PARAM_FORMAT, formats);
    if (err < 0) return err;
    ...
    for (prof = 0; prof < nprof; prof++) {
        if (0 == strncmp(basename, tfa_cont_profile_name(tfa98xx, prof), len)) {
            ...
            if (!sr) dev_info(component->dev, "Unable to identify supported sample rate\n");
    ...
    return snd_pcm_hw_constraint_list(substream->runtime, 0, SNDRV_PCM_HW_PARAM_RATE,
                                      &tfa98xx->rate_constraint);
}

static int tfa98xx_set_dai_sysclk(struct snd_soc_dai* codec_dai, int clk_id, unsigned int freq,
                                  int dir) {
...
static int tfa98xx_set_tdm_slot(struct snd_soc_dai* dai, unsigned int tx_mask, unsigned int rx_mask,
...
static int tfa98xx_hw_params(struct snd_pcm_substream* substream, struct snd_pcm_hw_params* params,
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void) {
    ...
    if (tfa98xx_device_count == bytes[0]) {
        ...
        if (1 == tfa98xx_device_count) bytes[0] = 0;
...
static int tfa98xx_mute(struct snd_soc_dai* dai, int mute, int stream) {
    ...
    if (mute) {
        ...
        if (stream == SNDRV_PCM_STREAM_PLAYBACK)
            ...
            tfa98xx->cstream = 0;
        if (tfa98xx->pstream != 0 || tfa98xx->cstream != 0) return 0;
        mutex_lock(&tfa98xx_mutex);
        tfa98xx_sync_count = 0;
        mutex_unlock(&tfa98xx_mutex);
        ...
        cancel_delayed_work_sync(&tfa98xx->init_work);
        if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) return 0;
        mutex_lock(&tfa98xx->dsp_lock);
...
static int tfa98xx_probe(struct snd_soc_component* component) {
    ...
    if (!tfa98xx->tfa98xx_wq) return -ENOMEM;
    ...
    dev_info(component->dev, "tfa98xx codec registered (%s) ret=%d", tfa98xx->fw.name, ret);
...
static void tfa98xx_remove(struct snd_soc_component* component) {
    ...
    if (tfa98xx->tfa98xx_wq) destroy_workqueue(tfa98xx->tfa98xx_wq);
}
...
static int tfa98xx_parse_dt(struct device* dev, struct tfa98xx* tfa98xx, struct device_node* np) {
    ...
    if (tfa98xx->reset_gpio < 0) dev_dbg(dev, "No reset GPIO provided, will not HW reset device\n");

    tfa98xx->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
    if (tfa98xx->irq_gpio < 0) dev_dbg(dev, "No IRQ GPIO provided.\n");
...
static int tfa98xx_i2c_probe(struct i2c_client* i2c, const struct i2c_device_id* id) {
    ...
    if (IS_ERR(tfa98xx->tfa98xx_vdd)) {
        ...
        if (regulator_count_voltages(tfa98xx->tfa98xx_vdd) > 0) {
            ...
            if (ret) {
                dev_err(&i2c->dev, "Regulator set tfa98xx_vdd failed ret=%d\n", ret);
                return ret;
            }
    ...
    if (np) {
        ...
        if (no_start) tfa98xx->irq_gpio = -1;
        if (no_reset) tfa98xx->reset_gpio = -1;
    } else {
    ...
    if (gpio_is_valid(tfa98xx->reset_gpio)) {
        ret = devm_gpio_request_one(&i2c->dev, tfa98xx->reset_gpio, GPIOF_OUT_INIT_LOW,
                                    "TFA98XX_RST");
        if (ret) return ret;
    }

    if (gpio_is_valid(tfa98xx->irq_gpio)) {
        ret = devm_gpio_request_one(&i2c->dev, tfa98xx->irq_gpio, GPIOF_DIR_IN, "TFA98XX_INT");
        if (ret) return ret;
    }
    ...
    if (tfa98xx->tfa == NULL) return -ENOMEM;
    ...
    if (!dai) return -ENOMEM;
    memcpy(dai, tfa98xx_dai, sizeof(tfa98xx_dai));

    tfa98xx_append_i2c_address(&i2c->dev, i2c, NULL, 0, dai, ARRAY_SIZE(tfa98xx_dai));

    ret = snd_soc_register_component(&i2c->dev, &soc_component_dev_tfa98xx, dai,
    ...
    if (no_start == 0) tfa98xx_debug_init(tfa98xx, i2c);

#ifdef CONFIG_DEBUG_FS
    tfa98xx_debugfs = debugfs_create_file(TFA98XX_DEBUG_FS_NAME, S_IFREG | S_IRUGO | S_IWUSR, NULL,
                                          (void*)TFA98XX_DEBUG_FS_NAME, &tfa98xx_debug_ops);
#else
    proc_create_data(TFA98XX_DEBUG_FS_NAME, S_IFREG | S_IRUGO | S_IWUSR, NULL, &tfa98xx_debug_ops,
    ...
    if (ret) dev_info(&i2c->dev, "error creating sysfs files\n");
...
static int tfa98xx_i2c_remove(struct i2c_client* i2c) {
    ...
    if (gpio_is_valid(tfa98xx->irq_gpio)) devm_gpio_free(&i2c->dev, tfa98xx->irq_gpio);
    if (gpio_is_valid(tfa98xx->reset_gpio)) devm_gpio_free(&i2c->dev, tfa98xx->reset_gpio);
...
static int __init tfa98xx_i2c_init(void) {
    ...
    tfa98xx_cache = kmem_cache_create(
            ...
            (SLAB_HWCACHE_ALIGN | SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD), /* Cache property */
