    ...
    if (result) {
        ...
        if (result == 0) {
            ...
            if (currentValue + delta < 15) {
                ...
                if (tfa->verbose) pr_debug("Max coil current is set to: 15 \n");
            }
        } else if (result == 1) {
            ...
            if (currentValue - delta > 0) {
                ...
                if (tfa->verbose) pr_debug("Max coil current is set to: 0 \n");
            }
        }
    }
...
static enum Tfa98xx_Error tfa9912_auto_copy_mtp_to_iic(struct tfa_device* tfa) {
    /* Set auto_copy_mtp_to_iic (bit 5 of A3) to 1. Workaround for 72, 88 and 9912/9892(see
     * PLMA5290) */
...
/* Maksimum value for combination of boost_voltage and vout calibration offset (see PLMA5322,
 * PLMA5528). */
...
static uint16_t tfa9912_vboost_fixup(struct tfa_device* tfa, uint16_t dcdc_cnt6) {
    ...
    if (boost_v_1st > TFA9912_VBOOST_MAX) boost_v_1st = TFA9912_VBOOST_MAX;

    if (boost_v_2nd > TFA9912_VBOOST_MAX) boost_v_2nd = TFA9912_VBOOST_MAX;
...
enum Tfa98xx_Error tfa9912_reg_write(struct tfa_device* tfa, unsigned char subaddress,
...
static enum Tfa98xx_Error tfa9888_specific(struct tfa_device* tfa) {
    ...
    if (patch_version >= 0x060401) tfa->partial_enable = 1;
...
static enum Tfa98xx_Error tfa9896_dsp_write_vsfwdelay_table(struct tfa_device* tfa) {
    return tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK, TFA1_FW_PAR_ID_SET_CURRENT_DELAY,
...
static enum Tfa98xx_Error tfa9896_dsp_write_cvfracdelay_table(struct tfa_device* tfa) {
    return tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK, TFA1_FW_PAR_ID_SET_CURFRAC_DELAY,
...
static enum Tfa98xx_Error tfa9894_specific(struct tfa_device* tfa) {
    ...
    if (tfa->in_use == 0) return Tfa98xx_Error_NotOpen;
    if (tfa->verbose)
        if (is_94_N2_device(tfa)) pr_debug("check_correct\n");
    /* Unlock keys to write settings */
    error = tfa_reg_write(tfa, 0x0F, 0x5A6B);
    error = tfa_reg_read(tfa, 0xFB, &value);
    xor = value ^ 0x005A;
