...
nxpTfaDeviceList_t* tfaContGetDevList_v6(nxpTfaContainer_t* cont, int dev_idx) {
    ...
    if ((dev_idx < 0) || (dev_idx >= cont->ndev)) return NULL;

    if (cont->index[dev_idx].type != dscDevice) return NULL;
    ...
    if (dev) {
        for (idx = 0, hit = 0; idx < dev->length; idx++) {
            if (dev->list[idx].type == dscProfile) {
                if (profIdx == hit++) return (nxpTfaProfileList_t*)(dev->list[idx].offset + base);
            }
        }
    }
...
int tfacont_get_max_vstep_v6(struct tfa_device* tfa, int prof_idx) {
    ...
    if (tfa->tfa_family == 2) {
        ...
        vp3 = (struct nxpTfaVolumeStepMax2File*)tfacont_getfiledata_v6(tfa, prof_idx, volstepHdr);
        if (vp3) {
...
nxpTfaFileDsc_t* tfacont_getfiledata_v6(struct tfa_device* tfa, int prof_idx,
    ...
    if (tfa->verbose) pr_info("%s: no file found of type %d\n", __FUNCTION__, type);
 ...
 */
static enum Tfa98xx_Error tfaContWriteVstep(struct tfa_device* tfa, nxpTfaVolumeStep2File_t* vp,
                                            int vstep) {
    ...
    if (vstep < vp->vsteps) {
        ...
        if (err != Tfa98xx_Error_Ok) return err;

        err = tfa98xx_dsp_write_preset_v6_v6(tfa, sizeof(vp->vstep[0].preset),
...
static struct nxpTfaVolumeStepMessageInfo* tfaContGetmsgInfoFromReg(
...
static int tfaContGetmsgLen(struct nxpTfaVolumeStepMessageInfo* msgInfo) {
    return (msgInfo->MessageLength.b[0] << 16) + (msgInfo->MessageLength.b[1] << 8) +
           msgInfo->MessageLength.b[2];
}

static struct nxpTfaVolumeStepMessageInfo* tfaContGetNextmsgInfo(
...
static struct nxpTfaVolumeStepRegisterInfo* tfaContGetNextRegFromEndInfo(
        struct nxpTfaVolumeStepMessageInfo* msgInfo) {
    char* p = (char*)msgInfo;
...
static struct nxpTfaVolumeStepRegisterInfo* tfaContGetRegForVstep(nxpTfaVolumeStepMax2File_t* vp,
                                                                  int idx) {
    int i, j, nrMessage;
...
static enum Tfa98xx_Error tfaContWriteVstepMax2_One(struct tfa_device* tfa,
     ...
     * 'without_reset' types inside the vstep. In other words: the reset msg is applied during
     * SBSL==0 else it remains unchanged.
     */
    if (tfa_needs_reset(tfa) == 1) {
        if (new_msg->MessageType == 0) {
            cmdid[2] = SB_PARAM_SET_ALGO_PARAMS;
            if (tfa->verbose) pr_debug("P-ID for SetAlgoParams modified!\n");
        } else if (new_msg->MessageType == 2) {
            cmdid[2] = SB_PARAM_SET_MBDRC;
            if (tfa->verbose) pr_debug("P-ID for SetMBDrc modified!\n");
        }
    }
    ...
    if (enable_partial_update) {
        ...
        if (!partial) pr_debug("Partial update memory error - Disabling\n");
    }
    ...
    if (partial) {
        ...
        *p++ = 0x00;
        ...
        while ((o < (old_msg->ParameterData + len)) && (p < (partial + len - 3))) {
            if ((offset == 0xff) || (memcmp(n, o, 3 * sizeof(uint8_t)))) {
                *p++ = offset;
                change = (uint16_t*)p;
                *change = 0;
                p += 2;

                for (i = 0; (i < 16) && (o < (old_msg->ParameterData + len)); i++, n += 3, o += 3) {
    ...
    if (partial) kmem_cache_free(tfa->cachep, partial);
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(struct tfa_device* tfa,
    ...
    if (tfa->p_regInfo == NULL) {
        if (tfa->verbose) pr_debug("Inital vstep write\n");
        enp = 0;
    }
    ...
    for (i = 0; i < nrMessages; i++) {
        ...
        if (msgInfo->MessageType == 3) {
            ...
            msgInfo = tfaContGetNextmsgInfo(msgInfo);
            if (enp) p_msgInfo = tfaContGetNextmsgInfo(p_msgInfo);
            continue;
        }
 ...
 */
enum Tfa98xx_Error tfaContWriteFile_v6(struct tfa_device* tfa, nxpTfaFileDsc_t* file, int vstep_idx,
                                       int vstep_msg_idx) {
    enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
    ...
    if ((type == msgHdr) || ((type == volstepHdr) && (tfa->tfa_family == 2))) {
        ...
        if ((subversion > 0) && (((hdr->customer[0]) == 'A') && ((hdr->customer[1]) == 'P') &&
                                 ((hdr->customer[2]) == 'I') && ((hdr->customer[3]) == 'V'))) {
            if (tfa->is_probus_device) {
                /* Temporary workaround (example: For climax --calibrate scenario for probus
                 * devices) */
                ...
                for (i = 0; i < 3; i++) {
                    if (tfa->fw_itf_ver[i] !=
                        hdr->customer[i + 4])  //+4 to skip "?PIV" string part in the .msg file.
                    {
                        ERRORMSG(
                                "Error: tfaContWriteFile: Expected FW API version = %d.%d.%d, Msg "
                                "File version: %d.%d.%d \n",
                                tfa->fw_itf_ver[0], tfa->fw_itf_ver[1], tfa->fw_itf_ver[2],
                                hdr->customer[4], hdr->customer[5], hdr->customer[6]);
                        return Tfa98xx_Error_Bad_Parameter;
                    }
                }
                ...
                ERRORMSG(
                        "Error: tfaContWriteFile: Expected FW API version = %d.%d.%d, Msg File "
                        "version: %d.%d.%d \n",
                        (tfa->fw_itf_ver[2]) & 0xff, (tfa->fw_itf_ver[1]) & 0xff,
                        (tfa->fw_itf_ver[0] >> 6) & 0x03, hdr->customer[4], hdr->customer[5],
                        hdr->customer[6]);
                return Tfa98xx_Error_Bad_Parameter;
            }
        }
    ...
    switch (type) {
        ...
        case volstepHdr:
            if (tfa->tfa_family == 2) {
                err = tfaContWriteVstepMax2(tfa, (nxpTfaVolumeStepMax2File_t*)hdr, vstep_idx,
                                            vstep_msg_idx);
            } else {
        ...
        case speakerHdr:
            if (tfa->tfa_family == 2) {
                ...
                err = tfa98xx_dsp_write_speaker_parameters_v6(
                        tfa, size, (const unsigned char*)((nxpTfaSpeakerFile_t*)hdr)->data);
            }
            break;
        case presetHdr:
            size = hdr->size - sizeof(nxpTfaPreset_t);
            err = tfa98xx_dsp_write_preset_v6_v6(
                    tfa, size, (const unsigned char*)((nxpTfaPreset_t*)hdr)->data);
            break;
        case equalizerHdr:
        ...
        case configHdr:
            ...
            err = tfa98xx_dsp_write_config_v6(tfa, size,
        ...
        case drcHdr:
            if (hdr->version[0] == NXPTFA_DR3_VERSION) {
                ...
                err = tfa98xx_dsp_write_drc_v6(
                        tfa, size, ((const unsigned char*)((nxpTfaDrc_t*)hdr)->data + 381));
            }
            break;
        case infoHdr:
            /* Ignore */
 ...
 */
static nxpTfaDescPtr_t* tfa_cnt_get_dsc(nxpTfaContainer_t* cnt, nxpTfaDescriptorType_t type,
                                        int dev_idx) {
...
int tfa_cont_get_idx(struct tfa_device* tfa) {
    ...
    for (i = 0; i < tfa->cnt->ndev; i++) {
        ...
        if (dev && (dev->dev == tfa->slave_address)) break;
    }
    if (i == tfa->cnt->ndev) return -1;
...
#ifdef TFA_DEBUG
    if (tfa->verbose)
        pr_debug("bitfield: %s=0x%x (0x%x[%d..%d]=0x%x)\n", tfaContBfName(bfUni.field, tfa->rev),
                 value, bfUni.Enum.address, bfUni.Enum.pos, bfUni.Enum.pos + bfUni.Enum.len, value);
#endif
...
static enum Tfa98xx_Error tfaRunWriteFilter(struct tfa_device* tfa, nxpTfaContBiquad_t* bq) {
    ...
    for (i = 0; i < runs; i++) {
        ...
        if (dmem == Tfa98xx_DMEM_ERR) {
            if (tfa->verbose) {
                pr_debug(
                        "Warning: XFilter settings are applied via msg file (ini filter[x] format "
                        "is skipped).\n");
            }
        ...
        if (tfa->tfa_family == 2) {
            error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK, FW_PAR_ID_SET_MEMORY,
                                            sizeof(data), data);
        } else {
            error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK, 4 /* param */, sizeof(data),
                                            data);
        }
    }
    ...
    if (tfa->verbose) {
        if (bq->aa.index == 13) {
            ...
            pr_debug("=%d,%.0f,%.1f,%.1f \n", bq->aa.type, bq->aa.cutOffFreq, bq->aa.rippleDb,
                     bq->aa.rolloff);
        } else {
...
enum Tfa98xx_Error tfaContWriteRegsDev_v6_v6(struct tfa_device* tfa) {
    ...
    for (i = 0; i < dev->length; i++) {
        if (dev->list[i].type == dscPatch || dev->list[i].type == dscFile ||
        ...
        if (dev->list[i].type == dscRegister) {
            err = tfaRunWriteRegister(tfa,
                                      (nxpTfaRegpatch_t*)(dev->list[i].offset + (char*)tfa->cnt));
        }
...
enum Tfa98xx_Error tfaContWriteRegsProf_v6(struct tfa_device* tfa, int prof_idx) {
    ...
    for (i = 0; i < prof->length; i++) {
        ...
        if (prof->list[i].type == dscDefault) break;
        ...
        if (prof->list[i].type == dscRegister) {
            err = tfaRunWriteRegister(tfa,
                                      (nxpTfaRegpatch_t*)(prof->list[i].offset + (char*)tfa->cnt));
        }
        if (err) break;
    }
    return err;
 ...
 */
static void create_dsp_buffer_msg(struct tfa_device* tfa, nxpTfaMsg_t* msg, char* buffer,
                                  int* size) {
    int i, nr = 0;
...
enum Tfa98xx_Error tfaContWriteFile_v6s(struct tfa_device* tfa) {
    ...
    for (i = 0; i < dev->length; i++) {
        ...
        if (dev->list[i].type == dscSetInputSelect || dev->list[i].type == dscSetOutputSelect ||
            dev->list[i].type == dscSetProgramConfig || dev->list[i].type == dscSetLagW ||
            dev->list[i].type == dscSetGains || dev->list[i].type == dscSetvBatFactors ||
            dev->list[i].type == dscSetSensesCal || dev->list[i].type == dscSetSensesDelay ||
            dev->list[i].type == dscSetMBDrc || dev->list[i].type == dscSetFwkUseCase ||
            dev->list[i].type == dscSetVddpConfig) {
            create_dsp_buffer_msg(tfa, (nxpTfaMsg_t*)(dev->list[i].offset + (char*)tfa->cnt),
                                  buffer, &size);
            if (tfa->verbose) {
                pr_debug("command: %s=0x%02x%02x%02x \n",
                         tfaContGetCommandString_v6(dev->list[i].type), (unsigned char)buffer[0],
        ...
        if (err != Tfa98xx_Error_Ok) break;

        if (dev->list[i].type == dscCfMem) {
            err = tfaRunWriteDspMem(tfa,
                                    (nxpTfaDspMem_t*)(dev->list[i].offset + (uint8_t*)tfa->cnt));
        }
 ...
 */
enum Tfa98xx_Error tfaContWriteFile_v6sProf_v6(struct tfa_device* tfa, int prof_idx,
                                               int vstep_idx) {
    enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
    nxpTfaProfileList_t* prof = tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
    char buffer[(MEMTRACK_MAX_WORDS * 3) + 3] = {0};  // every word requires 3 and 3 is the msg
        ...
        switch (prof->list[i].type) {
            ...
            case dscCfMem:
                err = tfaRunWriteDspMem(
                        tfa, (nxpTfaDspMem_t*)(prof->list[i].offset + (uint8_t*)tfa->cnt));
                break;
            ...
            case dscSetVddpConfig:
                create_dsp_buffer_msg(tfa,
                                      (nxpTfaMsg_t*)(prof->list[i].offset + (uint8_t*)tfa->cnt),
                                      buffer, &size);
                if (tfa->verbose) {
                    pr_debug("command: %s=0x%02x%02x%02x \n",
...
enum Tfa98xx_Error tfa_write_filters_v6(struct tfa_device* tfa, int prof_idx) {
    ...
    if (i == 0) {
        if (tfa->verbose) pr_err("Unable to write filters, CLKS=0 \n");
...
unsigned int tfa98xx_get_profile_sr_v6(struct tfa_device* tfa, unsigned int prof_idx) {
    ...
    if (tfa->verbose)
        pr_info("%s - profile fs: 0x%x = %dHz (%d - %d)\n", __FUNCTION__, fs_profile,
                tfa98xx_sr_from_field(fs_profile), tfa->dev_idx, prof_idx);

    if (fs_profile != -1) return tfa98xx_sr_from_field(fs_profile);
...
static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device* tfa, nxpTfaProfileList_t* prof,
    ...
    */
    ...
    if (fs_next_profile != fs_default_profile) {
        if (tfa->verbose) pr_debug("Writing delay tables for AUDFS=%d \n", fs_next_profile);
...
enum Tfa98xx_Error tfaContWriteProfile_v6(struct tfa_device* tfa, int prof_idx, int vstep_idx) {
    ...
    nxpTfaProfileList_t* prof = tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
    nxpTfaProfileList_t* previous_prof =
            tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, tfa_dev_get_swprof(tfa));
    char buffer[(MEMTRACK_MAX_WORDS * 4) + 4] = {
            0};  // every word requires 3 or 4 bytes, and 3 or 4 is the msg
    ...
    if (prof->group == previous_prof->group && prof->group != 0) {
         ...
         */
        if (((tfa->tfa_family == 2) && (TFA_GET_BF(tfa, MANSTATE) >= 6)) ||
            (tfa->tfa_family != 2)) {
    ...
    for (i = 0; i < prof->length; i++) {
        ...
        switch (prof->list[i].type) {
            ...
            default:
                ...
                if (err != Tfa98xx_Error_Ok) return Tfa98xx_Error_Bad_Parameter;
                break;
        }
    }

    if (prof->group != previous_prof->group || prof->group == 0) {
        if (tfa->tfa_family == 2) TFA_SET_BF_VOLATILE(tfa, MANSCONF, 1);
        ...
        if (tfa->tfa_family == 2) {
            ...
            TFA_SET_BF_VOLATILE(tfa, SBSL, 0);
            /* Sending commands to DSP we need to make sure RST is 0 (otherwise we get no
             * response)*/
            ...
            for (i = 0; i < previous_prof->length; i++) {
                ...
                if (i < previous_prof->length) {
                    if (previous_prof->list[i].type == dscFile ||
                        ...
                        file = (nxpTfaFileDsc_t*)(previous_prof->list[i].offset +
                                                  (uint8_t*)tfa->cnt);
                        err = tfaContWriteFile_v6(tfa, file, vstep_idx, TFA_MAX_VSTEP_MSG_MARKER);
                    }
                }
    ...
    for (i = j; i < prof->length; i++) {
        ...
        switch (prof->list[i].type) {
            ...
            case dscSetVddpConfig:
                ...
                if (tfa->ext_dsp != 0) {
                    create_dsp_buffer_msg(tfa,
                                          (nxpTfaMsg_t*)(prof->list[i].offset + (char*)tfa->cnt),
                                          buffer, &size);
                    err = dsp_msg(tfa, size, buffer);
...
char* tfaContGetString_v6(nxpTfaContainer_t* cnt, nxpTfaDescPtr_t* dsc) {
    if (dsc->type != dscString) return "Undefined string";
...
int tfa_cnt_get_app_name_v6(struct tfa_device* tfa, char* name) {
    ...
    for (i = 0; i < sizeof(tfa->cnt->application); i++) {
        ...
        if (tfa->cnt->application[i] == '\0') break;
    }
    name[len++] = '\0';
...
int tfaContGetCalProfile_v6(struct tfa_device* tfa) {
    ...
    if ((tfa->dev_idx < 0) || (tfa->dev_idx >= tfa->cnt->ndev)) return -1;
...
nxpTfaLiveDataList_t* tfaContGet1stLiveDataList_v6(nxpTfaContainer_t* cont) {
    ...
    while (maxprof != 0) {
        // get last proflist
        prof = (nxpTfaProfileList_t*)b;
        b += sizeof(nxpTfaProfileList_t) + ((prof->length - 1) * (sizeof(nxpTfaDescPtr_t)));
        maxprof--;
    }
...
nxpTfaProfileList_t* tfaContNextProfile_v6(nxpTfaProfileList_t* prof) {
    ...
    if (prof->ID != TFA_PROFID) return NULL; /* invalid input */
    ...
    if (nextprof->ID != TFA_PROFID) return NULL;
...
nxpTfaLiveDataList_t* tfaContNextLiveData_v6(nxpTfaLiveDataList_t* livedata) {
    nxpTfaLiveDataList_t* nextlivedata =
            (nxpTfaLiveDataList_t*)((char*)livedata + (livedata->length * 4) +
                                    sizeof(nxpTfaLiveDataList_t) - 4);

    if (nextlivedata->ID == TFA_LIVEDATAID) return nextlivedata;
...
static void get_all_features_from_cnt(struct tfa_device* tfa, int* hw_feature_register,
...
enum Tfa98xx_Error tfa_set_filters_v6(struct tfa_device* tfa, int prof_idx) {
    ...
    if (TFA_GET_BF(tfa, PWDN) == 1) return Tfa98xx_Error_Ok;
    ...
    for (i = 0; i < prof->length; i++) {
        ...
        if (prof->list[i].type == dscFilter) {
            if (tfaContWriteItem(tfa, &prof->list[i]) != Tfa98xx_Error_Ok) return err;
        }
    }
...
int tfa_tib_dsp_msgmulti(struct tfa_device* tfa, int length, const char* buffer) {
    ...
    if (blob == NULL) {
        if (tfa->verbose) pr_debug("%s, Creating the multi-message \n\n", __FUNCTION__);
